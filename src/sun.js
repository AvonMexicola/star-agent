...
void main(){
  ...
  float penumbra=(1.0-smoothstep(.20,.42,sc.x))*group,umbra=(1.0-smoothstep(.11,.22,sc.x))*group;
  ...
  color=mix(color,vec3(1.5,.28,.16)*1.6,(1.0-smoothstep(0.0,.06,mu)));
...
void main(){
  ...
  if(r>R*.985){
    ...
    vec3 tint=mix(vec3(1.0,.62,.30),vec3(.70,.80,1.0),smoothstep(R,R*2.4,r));
  ...
  for(int i=0;i<${PROMINENCE_COUNT};i++){
    ...
    // Fade before the local bounds; a hard cutoff makes rectangular flare cards.
    float envelope=(1.0-smoothstep(1.25,1.6,y/pr.z))*(1.0-smoothstep(1.5,2.2,abs(x)/pr.y));
    color+=pc*(ribbon+glow)*feet*1.9*pr.w*envelope;
  }
...
export const PROMINENCES = Object.freeze(Array.from({ length: PROMINENCE_COUNT }, (_, i) => Object.freeze({
  ...
  span: .07 + promRandom() * .12,
  height: .07 + promRandom() * .20,
...
export class Sun {
  constructor(scene) {
    ...
    this.sphere = new THREE.Mesh(new THREE.SphereGeometry(1, 192, 96), this.photosphere);
    this.sphere.scale.setScalar(SUN_RADIUS);
  ...
  update(worldPosition, camera, dt, elapsed, { atmosphereFraction = 0 } = {}) {
    ...
    if (near) {
      ...
      c.coronaStrength.value = THREE.MathUtils.lerp(.08, .24, smoothstep(SUN_EXCLUSION, SUN_EXCLUSION * 2.5, distance));
