...
export function createCargoVisual(){
  ...
  return {root,
    update(hull,crates,eye=new THREE.Vector3()){
      ...
      for(const key of new Set(crates.map(c=>`${c.sbu}:${c.resource}`))){const same=crates.filter(c=>`${c.sbu}:${c.resource}`===key),mesh=new THREE.InstancedMesh(labelGeometry,labelMaterial(same[0]),same.length);same.forEach((c,i)=>{const b=crateBounds(hull,c);mesh.setMatrixAt(i,new THREE.Matrix4().makeTranslation((b.min[0]+b.max[0])/2,b.min[1]+(b.max[1]-b.min[1])*.35,b.min[2]-.002));});mesh.computeBoundingSphere();root.add(mesh);}
