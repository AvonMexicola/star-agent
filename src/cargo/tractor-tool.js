...
export function createCargoTractor({scene,nav,api,ships,worldClear,getMuzzle}){
  ...
  return {equip,holster,secure,get held(){return held();},get active(){return Boolean(nav.tractorActive);},
    ...
    update(dt,origin){
      ...
      $('.tractor-target').textContent=c?`${c.recovery?'RECOVERY CARGO · ':c.transport?'SEALED FREIGHT · ':''}${c.sbu} SBU · ${c.resource.toUpperCase()}`:target?`${target.recovery?'RECOVERY CARGO · ':target.transport?'SEALED FREIGHT · ':''}${target.sbu} SBU · ${target.resource.toUpperCase()}`:'Aim at an SBU crate';
