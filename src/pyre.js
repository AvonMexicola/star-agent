...
export class Pyre {
  constructor(scene, { sync = false } = {}) {
    ...
    this.colorUniform={value:this.orbitalColor};this.normalUniform={value:this.orbitalNormal};
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace('#include <color_fragment>', `#include <color_fragment>
          vec3 pyD=normalize(vPyreDirection),pyN=normalize(vPyreNormal);
            ...
            pyN=normalize(texture2D(normalMap,vNormalMapUv).xyz*2.0-1.0);
          #endif
    ...
    this.terrain = new PyreTerrain(this.group, this.material, { sync, onMaps: data => {
      ...
      for(const [key,uniform,pixels,space] of [['orbitalColor',this.colorUniform,data.color,THREE.SRGBColorSpace],['orbitalNormal',this.normalUniform,data.normal,THREE.NoColorSpace]]){
        const map=new THREE.DataTexture(pixels,data.width,data.height);map.colorSpace=space;map.wrapS=THREE.RepeatWrapping;map.minFilter=THREE.LinearMipmapLinearFilter;map.magFilter=THREE.LinearFilter;map.generateMipmaps=true;map.needsUpdate=true;
        this[key].dispose();this[key]=map;uniform.value=map;
      }
