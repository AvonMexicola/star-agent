...
export function createSentrySystem({scene,canvas,nav,mining,effects,inventoryUI,multiplayer,enabled=false}){
  ...
  function nearest(){
    ...
    for(const s of snapshots())for(const [role,seat]of Object.entries(L.seats)){
      const point=v(seat.ground).applyQuaternion(new THREE.Quaternion(...s.quaternion)).add(v(s.position));
    ...
    return createSentrySimulation({id:'solo-sentry',ownerId:localId,...pose,sampleSupport:environment.support,referenceUp:environment.up,constrain:movement=>environment.constrain(movement,'solo-sentry'),accessClear:environment.accessClear,getCarriers:environment.carriers,getPlayer:id=>id===localId?player:null,getInput:()=>input,
      onSeat:()=>reset(),onFire:(poses,id,sequence,rover)=>{
        for(let barrel=0;barrel<poses.length;barrel++){
          ...
          renderer.fire({start:p.start.toArray(),end:end.toArray(),targetId:hit?.entity?.id??hit?.fauna?.id??null});
          if(hit){
  ...
  const api={touch,renderer,
    ...
    camera(camera){
      ...
      const q=new THREE.Quaternion(...c.quaternion),aim=q.clone().multiply(new THREE.Quaternion().setFromEuler(new THREE.Euler(c.pitch,c.yaw,0,'YXZ')));
      ...
      const sight=v(L.turret.sight).applyQuaternion(pitch).add(v(L.turret.pitch)).applyQuaternion(yaw).add(v(L.turret.yaw)).applyQuaternion(q).add(v(c.position));
      if(role==='pilot'&&c.seats.gunner.id){camera.position.copy(v(L.seats.pilot.eye).applyQuaternion(q).add(v(c.position)));camera.orientation.copy(q);}else{camera.position.copy(sight);camera.orientation.copy(aim);}
