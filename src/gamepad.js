...
const empty = () => ({ strafe: 0, forward: 0, yaw: 0, pitch: 0, vertical: 0, roll: 0,
  boost: false, jump: false, brake: false, speed: 0, scroll: 0, ui: null, pressed: new Set(), used: false });
...
export class GamepadInput {
  ...
  poll({ focused = true, enabled = true, ui = false } = {}) {
    ...
    if (!pad) {
      this.uiArmed = false; this.connected = false; this.index = null; this.id = null; this.previous = []; this.armed = false;
    ...
    if (this.index !== pad.index || this.id !== pad.id) {
      this.uiArmed = false; this.index = pad.index; this.id = pad.id; this.armed = false; this.previous = [];
    }
    this.connected = true;
    ...
    if (!ui || !focused || this.uiActive !== ui) this.uiArmed = false;
    this.uiActive = ui;
    if (ui && focused && neutral) this.uiArmed = true;
    ...
    if (!this.armed) {
      ...
      const result = empty();
      if (ui && this.uiArmed) result.ui = { x: strafe, y: moveY, pressed: new Set([...pressed].filter(i => [0, 1, 12, 13, 14, 15].includes(i))) };
      if(!enabled && !ui)result.scroll=clamp(lookY+Number(buttons[13]??false)-Number(buttons[12]??false),-1,1);
