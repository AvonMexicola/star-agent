...
export function generatePatch({face,level,ix,iy}) {
  ...
  function sample(u,v,sampleLevel) {
    const d=cubeDirection(face,u,v),h=terrainHeight(...d);
    const step=Math.max(.4,Math.min(200,(2/2**sampleLevel)*RADIUS/GRID*.5));
  ...
  function write(index,u,v,skirt=0) {
    const {d,h,color,normal}=sample(u,v,level),k=index*3;
  ...
  if(level>0){
    ...
    const parentVertex=(i,j)=>{
      const key=j*(GRID+1)+i;
      if(!cache.has(key)){
        const q=sample(pu0+parentSize*i/GRID,pv0+parentSize*j/GRID,level-1);
    ...
    for(let j=0;j<=GRID;j++)for(let i=0;i<=GRID;i++){
      const px=(ix%2)*GRID/2+i/2,py=(iy%2)*GRID/2+j/2;
      const x=Math.min(GRID-1,Math.floor(px)),y=Math.min(GRID-1,Math.floor(py)),fx=px-x,fy=py-y;
  ...
  return {center,positions,normals,colors,directions,waterPositions,heights,parentPositions,parentWaterPositions,parentNormals,parentColors,parentHeights,indices:new Uint16Array(indices)};
}
