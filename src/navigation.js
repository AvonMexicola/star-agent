...
export class Navigation {
  ...
  advanceFlight(dt,{moveForward=0,strafe=0,vertical=0,turn=0,tilt=0,roll=0,cruiseTarget=null}={}){
    ...
    const input=forward.clone().multiplyScalar(moveForward).addScaledVector(right,strafe);
      ...
      for(let i=0;i<steps;i++){
        ...
        if(toxic.hit){this.position.copy(toxic.point);const normal=bodySurfaceNormal(this.position,MIASMA);if(!this.crashAt(assessImpact(this.velocity,normal,'toxic regolith'),normal))this.touchDown();break;}
  ...
  update(dt){
    ...
    if(this.mode==='eva'){
      ...
      if(!this.kestrelAccess&&this.shipPosition&&canAttachRamp(this.toShipLocal(),this.doorProgress>.98,this.speed)){
    ...
    if(!spaceFlight&&!this.spaceParked&&!inertial&&!this.cabinFlight&&oldBody===this.body&&this.mode!=='landed'&&this.mode!=='crashed'&&this.mode!=='destroyed'){rotation.setFromUnitVectors(oldNormal,newNormal);this.orientation.premultiply(rotation).normalize();}
