...
export class Planet {
  ...
  receive(slot,data){
    ...
    const material=new THREE.MeshStandardMaterial({vertexColors:true,roughness:.96,metalness:0,side:THREE.DoubleSide});
  ...
  dispose(){this.albedoWorker.terminate();this.albedoUniform.value.dispose();this.surfaceTexture.dispose();for(const slot of this.workers)slot.worker.terminate();for(const n of this.nodes.values())if(n.mesh)this.disposeNode(n);this.landMaterial.dispose();this.waterMaterial.dispose();}
}
