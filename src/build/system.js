...
import { PIECES, GRID, STOREY, getLocalColliders } from './definitions.js';
import { getWorldBoxes, getPlacementBoxes, getPlacementBounds, capsuleIntersectsBox, constrainBuildStep } from './collision.js';
import { createBuildVisual, setDoorOpen } from './visuals.js';
...
export class BuildSystem {
  constructor({scene,nav,store,render=true}){
    ...
    if(render)Promise.all(Object.keys(PIECES).map(id=>createBuildVisual(id))).then(()=>{this.assetsLoaded=true;}).catch(e=>{this.error=`Building assets unavailable: ${e.message}`;});
  ...
  get state(){return {active:this.active,pieceId:this.pieceId,preview:this.preview?{pieceId:this.pieceId,valid:this.preview.valid,reason:this.preview.reason,cost:this.preview.cost,sources:this.preview.sources,position:this.preview.position,claimId:this.preview.claim?.id??null}:null,claims:structuredClone(this.claims),pieceCount:this.claims.reduce((s,c)=>s+c.pieces.length,0),error:this.error,grounded:this.grounded,assetsReady:this.models.size>0&&[...this.models.values()].every(m=>m.ready),materials:this.store.container('pack')?.items};}
  canBuild(){return !this.nav.openingActive&&this.nav.mode==='walk'&&!this.nav.insideShip&&!this.nav.dockedAtStation&&!this.nav.travel&&this.nav.altitude<80;}
    ...
    for(const c of this.claims){
      ...
      for(const p of c.pieces){
        if(this.render&&!this.models.has(p.id)){
          ...
          createBuildVisual(p).then(model=>{entry.group=model;entry.ready=true;model.position.fromArray(p.position);model.rotation.y=p.rotation;group.add(model);setDoorOpen(model,p.doorOpen?1:0);}).catch(e=>{this.error=`Building asset unavailable: ${e.message}`;});
        }
        const model=this.models.get(p.id)?.group;if(model){setDoorOpen(model,p.doorOpen?1:0);if(p.type==='mainframe')updateMainframeDisplay(model,c);}
  ...
  updateGhost(){
    if(!this.render||!this.preview)return;
    const key=this.pieceId;
    if(this.ghost.userData.piece!==key){
      this.ghost.userData.piece=key;this.ghost.traverse(o=>{o.geometry?.dispose();o.material?.dispose();});this.ghost.clear();
      for(const box of getLocalColliders(key,false)){
        const size=v(box.max).sub(v(box.min)),mesh=new THREE.Mesh(new THREE.BoxGeometry(size.x,size.y,size.z),new THREE.MeshBasicMaterial({color:0xb6efd1,transparent:true,opacity:.2,depthWrite:false}));mesh.position.copy(v(box.min).add(v(box.max)).multiplyScalar(.5));this.ghost.add(mesh);
      }
    }
    this.ghost.visible=true;this.ghost.position.fromArray(this.preview.position).sub(this._origin);
    this.boundary.visible=Boolean(this.preview.claim);if(this.preview.claim){this.boundary.position.fromArray(this.preview.claim.origin).sub(this._origin);this.boundary.quaternion.fromArray(this.preview.claim.quaternion);}
    ...
    this.ghost.traverse(o=>{if(o.material)o.material.color.set(this.preview.valid?0xb6efd1:0xffb56b);});
  }
  update(dt,origin){
    this._origin.copy(origin);this.sync();
    for(const c of this.claims){const group=this.groups.get(c.id);group.position.fromArray(c.origin).sub(origin);group.visible=v(c.origin).distanceTo(this.nav.position)<600;}
    if(this.active&&!this.canBuild())this.cancel();
    this.workLight.visible=this.active&&this.nav.enabled&&this.nav.focused;
  ...
  get interaction(){const hit=this.nearbyInteraction();return hit?`F / X · ${hit.p.type==='doorway'?(hit.p.doorOpen?'Close':'Open')+' base door':hit.p.type==='mainframe'?'Base mainframe':'Base storage'}`:'';}
  interact(){
    ...
    if(this.store.write({...this.store.state,build:data})){this.sync();this.nav.gamepad.suspend();this.nav.notify(piece.doorOpen?'Base door opened.':'Base door closed.');}else this.nav.notify(this.store.warning);return true;
  }
  constrainWalker(previous,proposed){
    ...
    for(const c of this.claims){if(c.body!==bodyAt(point).id||v(c.origin).distanceTo(point)>CLAIM_RADIUS+20)continue;
      const result=constrainBuildStep(this.toLocal(previous,c).toArray(),this.toLocal(point,c).toArray(),c.pieces,{eyeHeight:this.nav.layout?.eyeHeight??1.65,radius:.28,stepHeight:.3});
  ...
  raycast(start,direction,range=1600,envelope=null){
    ...
    for(const c of this.claims){if(v(c.origin).distanceTo(start)>Math.hypot(c.radius,CLAIM_HEIGHT)+range+(envelope?Math.max(...envelope.min.map(Math.abs),...envelope.max.map(Math.abs))*2:0))continue;
      ...
      for(const p of c.pieces)for(const b of getWorldBoxes(p)){
