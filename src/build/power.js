...
export function powerDemand(claim){return .25+claim.pieces.reduce((n,p)=>n+.01+(p.lightOn!==false?(p.type==='ceiling-light'?.05:p.type===FLOODLIGHT.id?FLOODLIGHT.powerKW:0):0)+(p.type==='terminal'?.15:p.type==='hangar-door'?.1:p.landingPad?.04:0),0);}
