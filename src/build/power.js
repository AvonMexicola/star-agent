...
export function validPower(p){return Boolean(p&&p.version===POWER_VERSION&&Number.isFinite(p.updatedAt)&&p.updatedAt>=0&&Number.isFinite(p.health)&&p.health>=0&&p.health<=100&&Number.isFinite(p.charge)&&p.charge>=0&&p.charge<=770&&Number.isFinite(p.unpoweredMs)&&p.unpoweredMs>=0&&p.fuel&&['uranium-ore','helium-3-regolith'].every(k=>Number.isFinite(p.fuel[k])&&p.fuel[k]>=0&&p.fuel[k]<=100));}
