...
export function planRemoval(build,storage,claimId,pieceId){
 ...
 const nextClaim={...claim,pieces,...(piece.type==='mainframe'?{useBuffer:false}:{})};if(claim.power)nextClaim.power={...claim.power,charge:Math.min(claim.power.charge,powerCapacity(nextClaim))};
 const next={...build,claims:!pieces.length?build.claims.filter(c=>c.id!==claim.id):build.claims.map(c=>c.id===claim.id?nextClaim:c)};
