...
export class BasePower {
 constructor({store,build,sandbox=false,now=Date.now}){Object.assign(this,{store,build,sandbox,now});this.last=0;this.cloud=null;this.statusCache=new WeakMap();}
 ...
 action(claimId,action,item){
  ...
  const c=advancePower(current,this.now(),t=>powerEnvironment(current,t)),container=this.store.container(coreId(c)),items={...container?.items};
  if(action==='fuel'){
  ...
  const ok=this.store.write(next);return {ok,message:ok?action==='fuel'?'Fuel loaded; surplus generation charges batteries.':'Base repaired by 25 health.':this.store.warning};
 }
}
