...
try {
  ...
  async function transit(name){
    ...
    while(performance.now()-started<limit){await new Promise(r=>setTimeout(r,150));if(performance.now()-started>1100 && planet.pending<4 && (name==='moon'?moon.terrain.maxLevel>=14:name==='pyre'?pyre.ready:name==='star'||name==='orbit'||name==='station'||planet.maxVisibleLevel>=12))break;}
  ...
  function updateHud(time){
    ...
    const nearMoon=nav.body.id==='selene',onPyre=nav.body.id==='pyre',onStar=nav.body.star===true;
    ...
    $('altitude-reference').textContent=onStar?'ABOVE PHOTOSPHERE':nearMoon?'ABOVE SELENE':onPyre?'ABOVE PYRE':'ABOVE AEON';
    ...
    $('terrain-status').textContent=onStar?'STELLAR OBSERVATION':nearMoon?`${moon.terrain.visibleCount} PATCHES · LOD ${moon.terrain.maxLevel}`:onPyre?`${pyre.terrain.visibleCount} PATCHES · LOD ${pyre.terrain.maxLevel}`:`${planet.visibleCount} PATCHES · LOD ${planet.maxVisibleLevel}`;
    lastHud=time;
  }
  ...
  function frame(time){
    ...
    document.body.classList.toggle('exploring',nav.body.star||altitude<12000||nav.mode!=='flight'||nav.stationDistance<2000);
