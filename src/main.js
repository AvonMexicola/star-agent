...
import {createPirates} from './pirates/system.js';
...
if(atlasMeadowStart&&SEED!==ATLAS_MEADOW_SEED){
  ...
  async function transit(name){
    ...
    if(name==='aeon-pirates'||name==='selene-pirates'){const site=pirates.sites.find(s=>s.id===name),point=site.approach,dir=point.clone().sub(new THREE.Vector3(...site.body.center)).normalize();if(site.bodyId==='selene')nav.transitMoon(35,dir.toArray());else nav.transit(dir.toArray(),35);nav.orientToward(site.ground(0,-5,1.5),new THREE.Vector3(...site.up));}
    else if(name==='star')nav.transitStar();
  ...
  const groundContracts=document.createElement('button');groundContracts.type='button';groundContracts.dataset.controllerKey='pirate-contracts';groundContracts.textContent='Ground pirate camps · Aeon / Selene';groundContracts.onclick=()=>{const current=document.querySelector('dialog[open]');if(current){current.addEventListener('close',()=>pirates.open(),{once:true});current.close();}else pirates.open();};document.getElementById('patrol-console').append(groundContracts);
  const gameplayMenu=createGameplayMenu({nav,dev:Boolean(devLauncher),screens:[
    ...
    {id:'contracts',label:'Contracts',dialogs:['patrol-console','pirate-console'],open:()=>combat.open()},
  ...
  function frame(time){
    ...
    if(onFoot&&!opening?.active){
      const eva=nav.mode==='eva',up=eva?new THREE.Vector3(0,1,0).applyQuaternion(nav.orientation):playerUp(nav),feet=nav.position.clone().addScaledVector(up,-(eva?SHIP_LAYOUT.eyeHeight:nav.walkEyeHeight));
      ...
      character.update(dt,{crouching:nav.crouching,moveX:nav.velocity.clone().applyQuaternion(nav.orientation.clone().invert()).x,moveZ:-nav.velocity.clone().applyQuaternion(nav.orientation.clone().invert()).z,speed:eva?0:nav.velocity.clone().projectOnPlane(up).length(),grounded:eva||nav.jumpHeight===0,jumping:!eva&&nav.jumpHeight>0,health:loadout.state.health/100,...miningTool.pose});
    }
    ...
    landmarks.update(origin,camera);mining.update(origin);basePower.update();baseCloud.update(dt);build.update(dt,origin);settlements.update(dt,origin);garages.update(origin);fauna.update(dt,origin);pirates.update(dt,origin);shipMiningInput.beforeUpdate();miningTool.update(dt,origin);trading.update(origin,dt);rover?.update(dt,origin);inventoryUI.update?.();loadoutBar.update();buildUI.update();
