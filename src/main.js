...
try {
  ...
  function frame(time){
    ...
    audio.update({throttle:Math.min(1,nav.engineAcceleration.length()/25),musicAltitude:nav.flightEnvironment.altitude,verticalSpeed:nav.velocity.dot(nav.normal),powered:nav.powered,speed:nav.shipSpeed,altitude,mode:nav.cabinFlight?'flight':nav.mode,boost:!nav.cabinFlight&&nav.boost,airless:nav.body.airless,inHangar,doorMotion:station.doorsOpen>0&&station.doorsOpen<1?1:0},dt);
    if(audioDebug)audioDebug.textContent=JSON.stringify({mode:nav.mode,audio:audio.enabled,music:audio.music?.state,engine:audio.engineAudio?.state,effects:audio.gameplay?.state},null,2);
    renderer.info.reset();
