...
if(atlasMeadowStart&&SEED!==ATLAS_MEADOW_SEED){
  ...
  function frame(time){
    ...
    landmarks.update(origin,camera);mining.update(origin);basePower.update();baseCloud.update(dt);build.update(dt,origin);settlements.update(dt,origin);fauna.update(dt,origin);pirates.update(dt,origin);shipMiningInput.beforeUpdate();miningTool.update(dt,origin);trading.update(origin,dt);rover?.update(dt,origin);inventoryUI.update?.();loadoutBar.update();buildUI.update();
