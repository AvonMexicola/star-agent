...
try {
  ...
  nav.onControllerInput=(pad,dt)=>{if(nav.openingActive)return;if(systemMap.open){systemMap.controllerInput(pad.ui);return;}if(pad.pressed.has(14)&&nav.mode!=='walk'&&nav.mode!=='eva'){systemMap.openMap();return;}controllerUI.update(pad,dt);};
