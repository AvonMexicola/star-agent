...
import { MiningField, ringSurveyPoint } from './mining/field.js';
...
try {
  ...
  const miningTool=createMiningTool({scene,camera,canvas,nav,rock:mining});
  ...
  async function transit(name){
    ...
    if(name==='ring'){nav.transitMoon();nav.position.copy(ringSurveyPoint()).add(new THREE.Vector3(0,0,42));nav.velocity.set(0,0,0);nav.orientToward(ringSurveyPoint(),new THREE.Vector3(0,1,0));}
    ...
    while(performance.now()-started<6500){await new Promise(r=>setTimeout(r,150));if(performance.now()-started>1100 && planet.pending<4 && (name==='moon'?moon.terrain.maxLevel>=14:name==='orbit'||name==='station'||name==='ring'||planet.maxVisibleLevel>=12))break;}
    $('transit').classList.remove('active');transiting=false;nav.enabled=true;
    notify(name==='ring'?'Ring survey. Brake to a stop, F leaves the chair; open the hatch and walk outside. G activates suit thrusters.':name==='moon'?'Selene descent. L lands; F leaves the chair. Open the rear hatch and walk down the ramp to explore.':name==='station'?'Station approach. W enters the bay; X brakes. Over the central pad, L docks.':name==='orbit'?'High orbit. Click to fly. W approaches Aeon; Space moves away.':'Arrival complete. Click to fly · L lands · F leaves the pilot chair.');
  }
  const ringButton=document.createElement('button');ringButton.type='button';ringButton.className=$('moon-destination')?.className??'destination';ringButton.dataset.destination='ring';ringButton.innerHTML='<strong>SELENE RINGS</strong><small>ASTEROID SURVEY · EVA</small>';document.querySelector('[data-destination="moon"]').after(ringButton);
  const controllerUI=createControllerUI({nav,openBackpack:()=>inventoryUI.openPack(),toggleTool:()=>miningTool.toggle(),destinations:[...document.querySelectorAll('[data-destination]')].map(button=>({id:button.dataset.destination,label:button.dataset.destination==='moon'?'Selene':button.dataset.destination==='ring'?'Selene rings':button.textContent.trim(),activate:()=>transit(button.dataset.destination),enabled:()=>!button.disabled}))});
