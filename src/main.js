...
try {
  ...
  function modelFor(id){
    if(!shipModels.has(id)){
      ...
      installLandingGear(model);model.visible=false;scene.add(model);weatherShip(model,planet.surfaceTexture);
  ...
  const graphicsSettings=createGraphicsSettings({nav,storage:localInventoryStorage,onChange:settings=>{
    ...
    automaticScale=settings.resolution==='auto';
    if(!automaticScale)renderScale=settings.resolution;
    resizePending=true;
  }});
  ...
  const controllerUI=createControllerUI({nav,actions:[
    {id:'free-drive',label:'N · Spool / disengage heading drive',activate:()=>nav.travel?nav.cancelTravel():nav.beginFreeTravel(),enabled:()=>Boolean(nav.travel)||nav.mode==='flight'},
    {id:'gear',label:'G · Deploy / retract landing gear',activate:()=>nav.toggleGear(),enabled:()=>nav.mode==='flight'&&!nav.autoland&&!nav.stationLift&&!nav.travel&&nav.powered},
    {id:'lights',label:'L · Toggle landing lights / flashlight',activate:()=>nav.toggleLights(),enabled:()=>nav.mode!=='crashed'},
    {id:'camera-view',label:'Toggle cockpit / third-person view',activate:()=>document.getElementById('camera-button').click(),enabled:()=>['flight','walk','landed'].includes(nav.mode)},
    {id:'graphics',label:'Graphics · grass distance and resolution',activate:()=>graphicsSettings.open()},{id:'crash-recover',label:'Return to orbit after crash',activate:()=>transit('orbit'),enabled:()=>nav.mode==='crashed'},{id:'fleet',label:'Fleet registry',activate:()=>fleetUI.openMenu()},{id:'power',label:'Toggle ship main power',activate:()=>nav.togglePower(),enabled:()=>nav.canTogglePower},...Object.entries(WEAPONS).map(([id,p])=>({id:`weapon-${id}`,label:`Ship weapon · ${p.label}`,activate:()=>flightEffects.select(id),enabled:()=>nav.mode==='flight'}))],openBackpack:()=>inventoryUI.openPack(),toggleTool:()=>miningTool.toggle(),openEquipment:()=>inventoryUI.openEquipment(),cycleEquipment:()=>miningTool.cycle(),cycleQuick:()=>{const r=loadout.selectQuick((loadout.state.quickIndex+1)%4);if(!r.ok)nav.notify(r.message);},useQuick:()=>useQuick(loadout.state.quickIndex),destinations:[...document.querySelectorAll('#quick-transit-menu [data-destination]')].map(button=>({id:button.dataset.destination,label:button.dataset.destination==='moon'?'Selene':button.dataset.destination==='ring'?'Selene rings':button.textContent.trim(),activate:()=>transit(button.dataset.destination),enabled:()=>!button.disabled})).concat(resourceRoutes.map(route=>({id:route.id,label:route.label,activate:()=>transit(route.id)})))});
  nav.onControllerInput=(pad,dt)=>{if(!firstReady||nav.openingActive)return;if(systemMap.open){systemMap.controllerInput(pad.ui);return;}if(pad.pressed.has(14)&&nav.mode==='flight'){systemMap.openMap();return;}controllerUI.update(pad,dt);flightEffects.controller(pad);};
  ...
  function updateHud(time){
    ...
    utilityStatus.textContent=(nav.mode==='walk'||nav.mode==='eva')?`LIGHT ${nav.flashlightOn?'ON':'OFF'} · L / MENU`:`GEAR ${nav.gearDeployed?'DOWN':'UP'} · LIGHTS ${nav.shipLightsOn?'ON':'OFF'} · N DRIVE`;
    ...
    const hints=controller?(onFoot?[['LS','MOVE'],['RS','LOOK'],['RT',loadout.item==='mining-laser-tool'?'MINE':'FIRE'],[eva?'A / B':'A',eva?'RISE / LOWER':'JUMP'],['X','INTERACT'],[eva?'LT':'Y',eva?'BRAKE':'SUIT'],['D-PAD ← / →','CYCLE / TOOL'],['↑ / ↓','QUICK / USE'],['VIEW','BACKPACK'],['MENU','COMMANDS']]:[['LS','MOVE'],['RS','LOOK'],['RT / LT','UP / DOWN'],['Y','LAND / LAUNCH'],['X','INTERACT / EVA'],['MENU','COMMANDS']]):onFoot?[['WASD','MOVE'],[eva?'SPACE / C':'SPACE',eva?'RISE / LOWER':'JUMP'],['T / MOUSE',loadout.item==='mining-laser-tool'?'MINE':'FIRE'],['F','INTERACT'],['I / K','PACK / GEAR'],['L','FLASHLIGHT'],['M','MAP']]:[['WASD','MOVE'],['SPACE / C','UP / DOWN'],['B','LAND / LAUNCH'],['N','DRIVE'],['G','GEAR'],['L','LIGHTS'],['F','INTERACT / EVA'],['M','MAP']];
    ...
    $('camera-button').disabled=nav.mode!=='flight'&&nav.mode!=='landed'&&nav.mode!=='walk';
    if(course){
    ...
    if(travel){
      ...
      $('travel-phase').textContent=`${travel.phase.toUpperCase()} · ${Number.isFinite(travel.eta)?travel.eta.toFixed(1)+'s':'0.9c MAX'} · ${travel.manual?'N':'X'} TO DISENGAGE`;
  ...
  function frame(time){
    ...
    character.setVisible(nav.mode==='walk'&&shipCamera.active);
    if(nav.mode==='walk'){
    ...
    ship.visible=Boolean(nav.shipPosition)||(nav.mode==='flight'&&(nav.locked||nav.controllerActive||shipCamera.engaged));
    if(ship.visible){
    ...
    if(drawScene)renderedFrames++;frames++;frameAccumulator+=realDt;if(frameAccumulator>=2){fps=Math.round(frames/frameAccumulator);frames=0;frameAccumulator=0;if(automaticScale&&(firstReady||preload.phase==='warmup')&&!transiting&&fps<23&&renderScale>.55){renderScale=Math.max(.55,renderScale*.85);resizePending=true;}}
