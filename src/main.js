...
try {
  ...
  function updateHud(time){
    ...
    const hintsNode=$(controller?'controller-hints':'keyboard-hints'),hintMode=`${controller}-${nav.roverOccupied}-${nav.mode}-${nav.berthRest}-${nav.insideShip}-${nav.dockedAtStation}-${loadout.item}-${nav.controllerShortcutModifier}`;
  ...
  function frame(time){
    ...
    character.setVisible(nav.mode==='walk'&&shipCamera.active&&!nav.kestrelAccess?.busy&&!nav.roverOccupied);
    if(nav.mode==='walk'){
    ...
    mining.update(origin);build.update(dt,origin);miningTool.update(dt,origin);rover?.update(dt,origin);inventoryUI.update?.();loadoutBar.update();buildUI.update();
