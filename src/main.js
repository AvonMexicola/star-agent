...
try {
  ...
  const reentry=new ReentryHeating(ship);
  ship.readyPromise?.then(()=>reentry.refresh());
  ...
  function frame(time){
    ...
    reentry.update({density:nav.flightEnvironment.density,velocity:nav.velocity,active:nav.mode==='flight',reset:transiting},dt,camera);
