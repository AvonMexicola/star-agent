...
if(atlasMeadowStart&&SEED!==ATLAS_MEADOW_SEED){
  ...
  const ensureRover=()=>{if(!rover)rover=createMiningRover({scene,canvas,nav,mining,effects,inventoryUI,getShip:()=>ship,available:()=>!multiplayer.connected,construction:roverConstruction,terrainObstacles:roverTerrainObstacles});nav.vehicle=rover;rover.bindCarrier();return rover;};
