...
if(atlasMeadowStart&&SEED!==ATLAS_MEADOW_SEED){
  ...
  const ensureRover=()=>{if(!rover)rover=createMiningRover({scene,canvas,nav,mining,effects,inventoryUI,getShip:()=>ship,available:()=>!multiplayer.connected&&(surfaceRoverStart||['atlas','gannet'].includes(nav.shipId))});nav.vehicle=vehicleRouter;rover.bindCarrier();return rover;};
  ...
  function frame(time){
    ...
    landmarks.update(origin,camera);mining.update(origin);basePower.update();baseCloud.update(dt);build.update(dt,origin);settlements.update(dt,origin);fauna.update(dt,origin);shipMiningInput.beforeUpdate();miningTool.update(dt,origin);trading.update(origin,dt);rover?.update(dt,origin);sentry.update(dt,origin);inventoryUI.update?.();loadoutBar.update();buildUI.update();
