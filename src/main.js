...
try {
  ...
  const introEnabled=!sandboxEnabled&&new URLSearchParams(location.search).get('intro')!=='0';
  ...
  function updateHud(time){
    ...
    const hints=controller?controllerHints({mode:nav.mode,insideShip:nav.insideShip,canBuild:canControllerBuild,tool:loadout.item}):onFoot?[['WASD','MOVE'],[eva?'SPACE / C':'SPACE',eva?'RISE / LOWER':'JUMP'],['T / MOUSE',loadout.item==='mining-laser-tool'?'MINE':'FIRE'],['F','INTERACT'],['I / K','PACK / GEAR'],['M','MAP']]:[['WASD','MOVE'],['SPACE / C','UP / DOWN'],['L','LAND / LAUNCH'],['F','INTERACT / EVA'],['M','MAP']];
    const hintsNode=$(controller?'controller-hints':'keyboard-hints'),hintMode=`${controller}-${nav.mode}-${nav.insideShip}-${canControllerBuild}-${loadout.item}`;
  ...
  function frame(time){
    ...
    if(nav.mode==='walk'){
      ...
      character.update(dt,{speed:nav.velocity.clone().projectOnPlane(up).length(),grounded:nav.jumpHeight===0,jumping:nav.jumpHeight>0});
    }
    ...
    mining.update(origin);build.update(dt,origin);miningTool.update(dt,origin);inventoryUI.update?.();loadoutBar.update();buildUI.update();
    ...
    if(!firstReady){
      ...
      if(preload.ready){
        ...
        if(sandboxEnabled){enterPlayerInterface();notify('Build sandbox · 4,608 kg supply bank. B / Menu → Build. Supplies tab refills the bank.');}
      }
    }
