...
if(atlasMeadowStart&&SEED!==ATLAS_MEADOW_SEED){
  ...
  const settlements=withPirateCompound(createSettlements({scene,nav,enabled:()=>!sandboxEnabled&&!multiplayer.connected}),pirateCompound);
  ...
  nav.baseAction=()=>build.interact();nav.baseInteraction=()=>build.interaction;pirateCompound.attachInteractions();
