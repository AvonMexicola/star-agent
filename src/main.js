...
try {
  ...
  const effects=new EnergyEffects(scene,{reducedMotion:matchMedia('(prefers-reduced-motion: reduce)').matches});
  ...
  const miningTool=createMiningTool({scene,camera,canvas,nav,rock:mining,effects});
  mining.onExtract=({point,yields,normal})=>effects.collect(point,yields,normal??nav.normal);
  const flightEffects=createFlightEffects({effects,nav,mining,camera});
  $('effects-motion').checked=effects.reducedMotion;
  $('effects-motion').addEventListener('change',()=>{effects.reducedMotion=$('effects-motion').checked;effects.reset();});
  $('effects-bloom').addEventListener('change',()=>atmosphere.bloom.enabled=$('effects-bloom').checked);
  const inventoryUI=createInventoryUI(nav,ship,inventory,mining.store);
  ...
  const controllerUI=createControllerUI({nav,openBackpack:()=>inventoryUI.openPack(),toggleTool:()=>miningTool.toggle(),actions:[...Object.entries(WEAPONS).map(([id,p])=>({id:`weapon-${id}`,label:`Ship weapon · ${p.label}`,activate:()=>flightEffects.select(id),enabled:()=>nav.mode==='flight'})),...['rifle-laser','sidearm-pistol','mining-laser-tool'].map((id,i)=>({id:`equip-${id}`,label:`Equip · ${['Solar carbine','Crimson sidearm','Mining cutter'][i]}`,activate:()=>miningTool.select(id),enabled:()=>(nav.mode==='walk'||nav.mode==='eva')&&!nav.insideShip}))],destinations:[...document.querySelectorAll('[data-destination]')].map(button=>({id:button.dataset.destination,label:button.dataset.destination==='moon'?'Selene':button.dataset.destination==='ring'?'Selene rings':button.textContent.trim(),activate:()=>transit(button.dataset.destination),enabled:()=>!button.disabled})).concat(resourceRoutes.map(route=>({id:route.id,label:route.label,activate:()=>transit(route.id)})))});
  nav.onControllerInput=(pad,dt)=>{controllerUI.update(pad,dt);flightEffects.controller(pad);};
  const menuButton=document.createElement('button');menuButton.id='commands-button';menuButton.type='button';menuButton.textContent='MENU';menuButton.title='Command menu · M / controller Menu';menuButton.addEventListener('click',()=>controllerUI.open());document.querySelector('.top-actions').prepend(menuButton);
  document.addEventListener('keydown',event=>{if(event.code==='KeyM'&&!event.repeat&&!event.target.closest('input,textarea,dialog'))controllerUI.open();});
  ...
  function updateHud(time){
    ...
    const toolAction=miningTool.state.item==='mining-laser-tool'?'MINE':'FIRE';
    const hints=controller?(onFoot?[['LS','MOVE'],['RS','LOOK'],['RT',toolAction],[eva?'A / B':'A',eva?'RISE / LOWER':'JUMP'],['X','INTERACT'],[eva?'LT':'Y',eva?'BRAKE':'SUIT'],['D-PAD →','TOOL'],['VIEW','BACKPACK'],['MENU','COMMANDS']]:[['LS','MOVE'],['RS','LOOK'],['RT / LT','UP / DOWN'],['A','FIRE'],['Y','LAND / LAUNCH'],['X','INTERACT / EVA'],['MENU','COMMANDS']]):onFoot?[['WASD','MOVE'],[eva?'SPACE / C':'SPACE',eva?'RISE / LOWER':'JUMP'],['T / MOUSE',toolAction],['F','INTERACT'],['I','BACKPACK'],['M','MENU']]:[['WASD','MOVE'],['SPACE / C','UP / DOWN'],['L','LAND / LAUNCH'],['F','INTERACT / EVA'],['M','MENU']];
    const hintsNode=$(controller?'controller-hints':'keyboard-hints'),hintMode=`${controller}-${nav.mode}-${toolAction}`;
    ...
    $('drive-label').textContent=nav.mode==='eva'?`EVA · ${nav.speed.toFixed(1)} m/s`:nav.mode==='walk'?'ON FOOT':nav.autoland?'AUTOLAND':nav.flightAssist?`ASSIST ×${nav.speedScale.toFixed(1)}`:'INERTIAL · V TO ASSIST';
  ...
  function frame(time){
    ...
    mining.update(origin);miningTool.update(dt,origin);inventoryUI.update?.();
    ...
    flightEffects.update(dt,origin,{suspended:transiting});
