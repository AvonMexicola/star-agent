...
export function createShipMFDs({ mounts = null, includeFrames = true, screenOffset = .034 } = {}) {
  ...
  group.update = (dt, nav, inventory, course) => {
    ...
    paint(screens[2], [['ENVIRONMENT', env.regime], [nav.freighter?'CARGO LIFTS':'HATCH / RAMP', nav.freighter?(nav.freighter.secured?'SECURED':'DEPLOYED'):nav.doorOpen ? nav.doorProgress > .98 ? 'OPEN / DEPLOYED' : 'OPENING' : nav.doorProgress > .02 ? 'CLOSING' : 'SEALED / STOWED'], Number.isFinite(nav.gearProgress)&&!nav.freighter?['LANDING GEAR', nav.gearProgress===1?'DOWN / LOCKED':nav.gearProgress===0?'STOWED':`${nav.gearDeployed?'DEPLOYING':'RETRACTING'} ${Math.round(nav.gearProgress*100)}%`]:['LOCAL VERTICAL', `${localVelocity.y.toFixed(1)} m/s`]], `ATMOSPHERE ${Math.round(env.atmosphereFraction * 100)}%   ${!nav.cabinFlight && nav.boost ? 'BOOST' : 'NOMINAL'}`, 2);
    paint(screens[3], [['SHIP STORAGE', `${inventory.mass('ship').toFixed(1)} / ${(inventory.capacity?.ship ?? CAPACITY.ship)} kg`], ['BACKPACK', `${inventory.mass('pack').toFixed(1)} / ${(inventory.capacity?.pack ?? CAPACITY.pack)} kg`], ['ACCESS', nav.freighter ? 'CARGO DECK' : 'AFT RACK / PORT']], 'ON FOOT: F AT THE CARGO CONTAINER', 3);
  };
