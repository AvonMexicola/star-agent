...
export function createShipMarker({nav,camera,entryLocal,shipName='Nomad',parent=document.body}){
  ...
  return {
    update(width,height){
      ...
      detail.textContent=projection.behind?'YOUR SHIP · TURN BACK':projection.distance<75?(nav.doorOpen?'REAR RAMP · APPROACH SLOWLY':'REAR HATCH · CLOSED'):'YOUR SHIP · BEACON';
      element.setAttribute('aria-label',`Your ship, ${shipName}, ${markerDistance(projection.distance)} to rear ramp${projection.behind?', behind you':''}`);
    },
