...
export class Vegetation {
  ...
  rebuildTrees() {
    let trees=0;const lodCounts=[0,0,0];
    ...
    for(let level=0;level<3;level++)for(const mesh of this.treeMeshes[level]){mesh.count=lodCounts[level];this.upload(mesh);}
    Object.assign(this.stats,{trees,treeLods:lodCounts,treeRange:1400,rebuilds:this.stats.rebuilds+1});
  }
