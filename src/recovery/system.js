...
export function createRecoverySystem({scene,nav,trading,combat,settlements}){
  ...
  const api={nav,get grounded(){return false;},constrainWalker:(_a,b)=>({point:b,hit:false,grounded:false}),
    ...
    get beacons(){const m=mission();if(!m)return [];const j=recoveryJob(m.job);return [{id:`wreck-${m.id}`,name:`${j.name} · disabled Atlas`,kind:'Deep-space recovery · open aft bay',summary:`${j.crates} sealed crates · ${j.reward} CR · ${m.cleared?'Recovery authorized':'Defending flight'}`,category:'missions',parent:'star',body:'aeon',center:[...m.position],radius:0},...trading.api.snapshot().loose.filter(c=>c.recovery?.id===m.id).map(c=>({id:`recovery-crate-${c.id}`,name:'Your recovery crate · 2 SBU',kind:'Personal recovery cargo',category:'missions',parent:'star',center:[...c.position],radius:0}))];},
