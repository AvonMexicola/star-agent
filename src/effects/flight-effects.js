...
export function createFlightEffects({effects,nav,mining,camera}){
  ...
  return {
    ...
    update(dt,origin,{suspended=false}={}){
      ...
      panel.hidden=nav.shipId==='kestrel'||nav.mode!=='flight'||Boolean(document.querySelector('dialog[open]'));
      if(!active)clear();
      ...
      if(active&&nav.shipId!=='kestrel'&&(keyHeld||pointerHeld||(controllerArmed&&controllerFire))&&cooldown===0){
