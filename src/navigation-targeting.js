...
export function createNavigationTargeting({nav,camera,destinations,station,build,multiplayer,combat,vehicleTargets=()=>[],parent=document.body}) {
  ...
  function route(target,{charged=false}={}){
    ...
    return planNavigationTravel(nav.position,target,{obstacles:obstacles(),rotationTime:nav.rotationClock?nav.rotationTime:null,spoolSeconds:charged?0:null});
  }
  function select(id){
  ...
  function engage(){
    ...
    const current=aimedNavigationTarget(nav.position,nav.orientation,targets().filter(t=>t.category==='bodies'||filters[t.category]||t.id===selectedId),selectedId,{rotationTime:nav.rotationClock?nav.rotationTime:null});
    ...
    const planned=route(current,{charged:true});if(!planned.ok){reset();nav.notify(planned.reason);return false;}
  ...
  return {targets,select,route,update,engage,reset,clear(){selectedId=null;nav.travelTarget=null;reset();},
    ...
    get course(){const target=targets().find(t=>t.id===selectedId)??aimed;return target?{name:target.name,point:nav.viewPoint(new Vector3(...target.center))}:null;},
