...
export function createDevLauncher({nav,options,seed,publicBuild=false,available=()=>true}){
  ...
  if(publicBuild){dialog.querySelector('.dev-eyebrow').textContent='SOLO EXPLORER / DEVELOPMENT SNAPSHOT';dialog.querySelector('.dev-intro').textContent='Your machine runs the universe. Choose a ship and a starting point, inspect the content, or test a system. Test sessions stay separate from your regular browser save.';}
