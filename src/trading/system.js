...
export function createTradingSystem({scene,nav,station,store,multiplayer,getShip,build,mining,remotePlayers,getMuzzle}){
  ...
  const api={callShip:hull=>multiplayer.request('cargoHull',{hull}),snapshot,atTerminal,nearestTerminal,docked,canTake,canStow,sources,
    ...
    async command(m){
      ...
      return local.command(fields,{terminal:atTerminal,docked,resources:id=>api.loose(m.source,id),crate:canTake,grid:canStow,loot:()=>false,tractor:tractorContext({nav,ships:physicalShips,loose:()=>snapshot().loose,worldClear})});
    },
  ...
  return {ui,api,tractor,
    ...
    },dispose(){tractor.dispose();ui.dispose();pads.dispose();for(const v of visuals.values())v.dispose();for(const a of accessModels.values())a.dispose();carry.removeFromParent();},
  };
}
