...
import { tradeSummary } from './base-stock.js';
...
export function createTradingSystem({scene,nav,station,store,multiplayer,getShip,build,mining,remotePlayers,getMuzzle,stationMarket=marketIdForTerminal}){
  ...
  const api={nav,localBases:()=>store.state.build?.claims??[],baseActive:t=>!t.base||t.base.shared||!build.power||Boolean(build.claims.find(c=>c.id===t.base?.claim.id)&&build.power.status(build.claims.find(c=>c.id===t.base.claim.id)).powered),callShip:hull=>multiplayer.request('cargoHull',{hull}),snapshot,atTerminal,nearestTerminal,docked,canTake,canStow,sources,
    ...
    async command(m){
      ...
      return local.command(fields,{nav,baseActive:api.baseActive,terminal:atTerminal,docked,stationMarket,resources:id=>api.loose(m.source,id),crate:canTake,grid:canStow,loot:()=>false,tractor:tractorContext({nav,ships:physicalShips,loose:()=>snapshot().loose,worldClear})});
    },
