...
export function createTradingSystem({scene,nav,station,store,multiplayer,getShip,build,mining,remotePlayers,getMuzzle,settlements,stationMarket=marketIdForTerminal,extraShips=()=>[],extraBeacons=()=>[],missionContext=()=>({})}){
  ...
  const physicalShips=()=>[...snapshot().ships.filter(s=>pose(s)).map(s=>{const own=s.owner===snapshot().owner,peer=multiplayer.state.players.find(p=>p.id===s.owner);return {...s,pose:pose(s),speed:own?nav.shipSpeed:Math.hypot(...(peer?.shipVelocity??[0,0,0])),open:own?nav.doorProgress>.98:(peer?.doorProgress??0)>.98,systems:own?nav.freighter:peerLifts.get(s.owner)};}),...extraShips()];
  ...
  nav.cargoEVA=(a,b)=>constrainCargoEVA(a,b,physicalShips());
