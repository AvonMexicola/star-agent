...
export function createFleetUI(nav, fleet, selectShip) {
  ...
  function render() {
    ...
    dialog.querySelector('.fleet-ships').innerHTML=Object.entries(SHIPS).map(([id,ship])=>`<article><span>${id==='atlas'?'02 / HEAVY LOGISTICS':'01 / SURVEY'}</span><h3>${ship.name}</h3><p>${ship.description}</p><button data-ship="${id}" ${!canSelect||!fleet.allows(id)||nav.shipId===id?'disabled':''}>${nav.shipId===id?'ACTIVE SHIP':fleet.allows(id)?'BOARD '+ship.name.toUpperCase():'LOCKED'}</button></article>`).join('');
