...
import {piratePropPlacements,crimsonLampEmitter} from './prop-layout.js';
...
export class PirateProps{
 constructor(scene,layout){this.layout=layout;this.group=new T.Group();this.group.name=layout.id==='pirate-hush'?'Crimson salvage fixtures':`${layout.name} · Crimson salvage fixtures`;this.scene=scene;this.origin=v(layout.claim.origin);this.q=new T.Quaternion(...layout.claim.quaternion);this.inverse=this.q.clone().invert();this.ready=false;this.error='';this.disposed=false;this.instances=[];this.models=[];this.lights=createFloodlights(scene);this.placements=piratePropPlacements(layout.deck);scene.add(this.group);
