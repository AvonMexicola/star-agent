...
export function createPirateCompound({scene,nav,getCombat,enabled=()=>!nav.multiplayer?.connected}){
 ...
 const policy=new PiratePerimeter({onWarning:()=>nav.notify('HUSH PERIMETER · Aircraft prohibited within180m. Leave beyond220m. Land at the outer amber pad and approach on foot.'),onShot:({damage,barrel})=>{
  const start=tower.muzzle(barrel),direction=tower.direction(),end=start.clone().addScaledVector(direction,P.disengage),position=nav.position;
  const radius=SHIP_STATS[nav.shipId]?.radius??SHIP_STATS.nomad.radius,t=segmentSphere(start,end,position,radius);
  const wall=nav.buildingRaycast?.(start,direction,P.disengage);let hit=t!==null&&(!wall||wall.distance>=start.distanceTo(end)*t);
 ...
 const panelPoint=point(layout.panel),nearPanel=()=>enabled()&&nav.mode==='walk'&&!nav.insideShip&&!nav.buildActive&&nav.position.distanceTo(panelPoint)<2.8;
 ...
 const open=()=>{if(!nearPanel()||document.querySelector('dialog[open]'))return false;nav.keys.clear();nav.gamepad.suspend();nav.enabled=false;render();dialog.showModal();return true;};
 ...
 const api={layout,tower,policy,buildings,nav,get layouts(){return [layout];},get claims(){return enabled()?claims:[];},get grounded(){return enabled()&&collision.grounded;},
  ...
  update(dt,origin){const on=enabled(),near=on&&nav.position.distanceTo(v(layout.claim.origin))<2400;if(near!==visible){visible=near;store.state.build={...emptyBuild(),claims:near?claims:[]};}buildings.update(dt,origin);
   ...
   const aircraft=nav.mode==='flight',ship=nav.position,targetLocal=local(ship),distance=ship.distanceTo(tower.origin),aligned=!suspended&&aircraft?tower.aim(ship,Math.min(.1,dt)):false;
   const start=tower.ready?tower.muzzle(0):tower.origin,delta=ship.clone().sub(start),wall=!suspended&&near&&aircraft&&distance<P.engage&&delta.length()>1e-6?nav.buildingRaycast?.(start,delta.clone().normalize(),delta.length()):null;
   props.update(nav.position,origin,on);
   policy.update(dt,{distance,altitude:targetLocal.y-layout.deck,ship:aircraft,active:!suspended,clear:!wall||wall.distance>delta.length()-12,aligned});tower.update(Math.min(.1,dt),origin,{enabled:on,phase:policy.phase});
...
export function withPirateCompound(settlements,pirate){return {
 ...
 terminalPosition:id=>pirate.terminalPosition(id)??settlements.terminalPosition(id),docked:(id,pose)=>id===PIRATE_MARKET.id?pirate.docked(id,pose):settlements.docked(id,pose),beacons:()=>[...settlements.beacons(),...pirate.beacons()],
