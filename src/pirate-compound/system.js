...
export function createPirateCompound({scene,nav,mining,getCombat,enabled=()=>!nav.multiplayer?.connected}){
 ...
 const policy=new PiratePerimeter({onWarning:()=>nav.notify('HUSH PERIMETER · Aircraft prohibited within 180 m. Leave beyond 220 m. Land at the outer amber pad and approach on foot.'),onShot:({damage,barrel})=>{
 ...
 const api={layout,tower,policy,buildings,nav,get layouts(){return [layout];},get claims(){return enabled()?claims:[];},get grounded(){return enabled()&&collision.grounded;},
  ...
  docked(id,pose){if(!enabled()||!policy.disabled||id!==PIRATE_MARKET.id||!pose)return false;const p=local(pose.position);return collision.landingSurface({...pose,orientation:pose.orientation??pose.quaternion})&&Math.abs(p.y-layout.padDeck)<.6&&Math.abs(p.x)<24&&Math.abs(p.z-260)<36;},
