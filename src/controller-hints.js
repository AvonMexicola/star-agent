...
export function controllerHints({mode,insideShip=false,dockedAtStation=false,canBuild=false,tool='mining-laser-tool'}){
  const menus=[['VIEW','BACKPACK'],['MENU','COMMANDS'],['LB + RB','SHORTCUTS']];
  if(tool==='cargo-tractor'&&['walk','eva'].includes(mode))return [['LS','MOVE'],['RS','GUIDE CRATE'],['RT','TRACTOR'],['X','SECURE GRID'],['↑ / ↓','HOLD DISTANCE'],['←','ALIGN TO SHIP'],['→','HOLSTER'],...menus];
