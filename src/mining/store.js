import { ROCK_ID, ROCK_VERSION, SIDE, createDensity } from './volume.js';
...
export class MiningStore {
  ...
  write(next){
    ...
    try{if(!this.storage)throw Error('Browser storage unavailable');this.storage.setItem(MINING_KEY,JSON.stringify({...next,field:Array.from(next.field)}));this.saved=true;this.warning='';}
  ...
  commit(result,revision){
    ...
    return this.write({...this.state,field:result.field,revision:revision+1,pack:this.state.pack.map((v,i)=>v+added[i])});
  }
