...
export function createMiningTool({scene,camera,canvas,nav,rock,effects=null,loadout=null,character=null,thirdPerson=()=>false,getBuild=()=>null}){
  ...
  function select(slot){if(nav.buildActive)return;nav.onTractorHolster?.({clearLoadout:false});clear();nav.gamepad.suspend();if(slot!==null&&isHandsFree(nav)){nav.notify(HANDS_FREE_REASON);return;}if(loadout){const result=loadout.select(slot);if(!result.ok)nav.notify(result.message);}else selected=slot!==null;}
  function cycle(){if(nav.buildActive)return;nav.onTractorHolster?.({clearLoadout:false});clear();nav.gamepad.suspend();if(isHandsFree(nav)){nav.notify(HANDS_FREE_REASON);return;}if(loadout){const result=loadout.cycle();if(!result.ok)nav.notify(result.message);}}
  ...
  return {
    ...
    update(dt,origin){
      ...
      if(portrait&&isBuilding)mount.position.set(.10,-.005,-1+recoil);
      else if(portrait&&isMining)mount.position.set(.12,-.25,-.65+recoil);
      else mount.position.set(compact?.25:.29,compact?-.25:-.35,-.47+recoil);
      mount.position.applyQuaternion(nav.orientation);mount.position.add(nav.position.clone().sub(origin));mount.quaternion.copy(nav.orientation);mount.updateMatrixWorld(true);
