...
export function createMiningTool({scene,camera,canvas,nav,rock,effects=null}){
  ...
  let hit=null,held=false,keyHeld=false,controllerArmed=false,selected=true,active=false,item='mining-laser-tool',recoil=0,direction=new THREE.Vector3();
  ...
  function loadItem(id){equipment.equip(id).then(()=>{if(item!==id)equipment.unequip(id);});}
  loadItem('mining-laser-tool');
  ...
  const clear=()=>{held=false;keyHeld=false;controllerArmed=false;rock.budget=0;};
  ...
  function selectItem(id){
    if(!['rifle-laser','sidearm-pistol','mining-laser-tool'].includes(id))return;
    clear();nav.gamepad.suspend();selected=true;
    if(item!==id){item=id;loadItem(id);}
    if(effects)effects.miningInput=null;
    for(const b of panel.querySelectorAll('[data-field-item]'))b.setAttribute('aria-pressed',String(b.dataset.fieldItem===item));
  }
  for(const b of panel.querySelectorAll('[data-field-item]'))b.addEventListener('click',()=>selectItem(b.dataset.fieldItem));
  document.addEventListener('keydown',e=>{
    if(!active||e.repeat||e.target.closest('dialog,input'))return;
    const id={Digit1:'rifle-laser',Digit2:'sidearm-pistol',Digit3:'mining-laser-tool'}[e.code];
    if(id)selectItem(id);if(e.code==='KeyR'){selected=!selected;clear();nav.gamepad.suspend();}
    if(e.code==='KeyT')keyHeld=true;
  });
  document.addEventListener('keyup',e=>{if(e.code==='KeyT')keyHeld=false;});
  return {
    equipment,select:selectItem,
    toggle(){selected=!selected;clear();nav.gamepad.suspend();},
    update(dt,origin){
      const isMining=item==='mining-laser-tool';
      ...
      mount.position.set(.29,-.35,-.47+recoil).applyQuaternion(nav.orientation);mount.quaternion.copy(nav.orientation);mount.updateMatrixWorld(true);
      ...
      if(active&&nav.toolTrigger<=.1)controllerArmed=true;
      const firing=active&&selected&&Boolean(held||keyHeld||(controllerArmed&&nav.toolTrigger>.1))&&(!isMining||(rock.store.free>.001&&!rock.error&&!rock.store.blocked));
      ...
      if(effects&&!isMining&&equipment.firingInput()){
        const start=equipment.muzzleWorldPosition(),range=item==='rifle-laser'?1200:450;
        if(start){
          const aimed=weaponTarget(nav.position,direction,origin,range),end=aimed?.point??nav.position.clone().addScaledVector(direction,range);
          const ray=end.clone().sub(start).normalize(),contact=weaponTarget(start,ray,origin,range);
          effects.fire(start,ray,{hit:contact,range,power:item==='rifle-laser'?.28:.2,color:item==='rifle-laser'?0xff902d:0xff395f});
          recoil=item==='rifle-laser'?.065:.04;
        }
      }
      if(effects){
      ...
      if(!active)return;
      for(const b of panel.querySelectorAll('[data-field-item]'))b.setAttribute('aria-pressed',String(b.dataset.fieldItem===item&&selected));
      $('.mining-heat').hidden=!isMining;button.textContent=isMining?'HOLD TO MINE':'HOLD TO FIRE';
      if(!isMining){
        $('.mining-eyebrow').textContent='SUIT / ENERGY WEAPON';
        $('.mining-target').textContent=item==='rifle-laser'?'SOLAR CARBINE':'CRIMSON SIDEARM';
        $('.mining-guide').textContent=selected?'Aim at rock or terrain · Energy impacts':'Weapon holstered';
        $('.mining-resources').textContent=item==='rifle-laser'?'Amber pulses · Automatic fire':'Crimson pulses · Automatic fire';
        $('.mining-feedback').textContent=nav.controllerActive?'RT · Fire / D-pad → · Holster / Menu · Equipment':'Hold T / mouse · Fire / 1–3 · Equip / R · Holster';
        button.disabled=!selected;return;
      }
    ...
    get state(){const mountedItems=[];hand.traverse(object=>{if(object.name.startsWith('equipment-'))mountedItems.push(object.name.slice(10));});return {active,item,mountedItems,firing:equipment.firingInput(),hit:hit?.point.toArray()??null,heat:equipment.heat,beaming:equipment.beaming,selected,target:rock.inspectState??null,toolError:equipment.error};},
  };
}
