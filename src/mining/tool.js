...
export function createMiningTool({scene,camera,canvas,nav,rock,effects=null,loadout=null,character=null,thirdPerson=()=>false}){
  ...
  return {
    ...
    update(dt,origin){
      ...
      active=!nav.carryingCargo&&!nav.buildActive&&(nav.mode==='walk'||nav.mode==='eva')&&!nav.insideShip&&nav.enabled&&nav.focused&&!document.hidden&&!document.querySelector('dialog[open]');
      panel.hidden=!active;
