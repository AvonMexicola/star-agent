...
export function createRoom({world,store,now=Date.now,autoStart=true,onError=()=>{}}) {
  ...
  const trading=createTrading({store,players,world,persistent,now,canTractor:p=>hub.canFire(p),flushWrites:p=>writes.get(p.id)??Promise.resolve()});
  ...
  const publicState=()=>({stationFrame:world.station?{direction:world.station.direction.toArray(),orientation:world.station.baseQuaternion.toArray(),altitude:world.station.altitude}:null,players:Array.from(players.values(),playerSnapshot),sentries:sentries.snapshot(),doors:{...doors},defense:world.defense?.snapshot??[]});
  function state(p,shared=publicState()){
    ...
    return {type:'state',...shared,hangar:hangar(p),hub:hub.snapshot(p),inventory:p.inventory,commerce:trading.snapshot(p),health:p.health,drops:nearby};
  }
