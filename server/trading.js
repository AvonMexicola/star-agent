...
export function createTrading({store,players,world,persistent,flushWrites,now=Date.now,stationMarket=marketIdForTerminal}){
  ...
  function context(p,ledger){return {
    ...
    docked:(s,id)=>{const n=players.get(s.owner)?.nav;return s.owner===p.id&&n&&n.shipId===s.hull&&!n.cabinFlight&&n.shipVelocity.length()<1&&!n.travel&&(id.startsWith('station:')?stationedForTrade({nav:n,hangarId:p.hangarId,station},id):onTradePad(pose(s)?.position,ledger.terminals[id]));},
