...
export function createSentries({players,world,send,impact,security,broadcast,canFire=()=>true,now=Date.now}){
  ...
  function vehicleHit(origin,direction,range){
    let result=null;for(const r of vehicles.values()){
      const distance=sentryBodyDistance(origin,direction,r.physics.state.position,r.physics.state.quaternion);
  ...
  function fire(poses,shooterId,sequence,rover){
    ...
    for(let barrel=0;barrel<poses.length;barrel++){
      ...
      const vehicle=vehicleHit(start,direction,distance);if(vehicle){distance=vehicle.distance;if(vehicle.id!==rover.id&&vehicle.health>0){victim=vehicle;kind='vehicle';}}
      for(const p of players.values()){
        const ship=shipPose(p);
        if(ship){const d=shipDistance(start,direction,ship.position,ship.rotation,ship.bounds);if(d<distance){distance=d;victim=p.id!==shooterId&&p.shipHealth>0?p:null;kind=victim?'ship':null;}}
        if(p.id===shooterId||rover.occupant(p.id)||p.health<=0||!['walk','eva'].includes(p.nav.mode))continue;
        const d=capsuleDistance(start,direction,p.nav.position,playerUp(p.nav));if(d<distance){distance=d;victim=p;kind='player';}
      }
      const point=start.clone().addScaledVector(direction,distance),packet={type:'event',event:'sentryFire',roverId:rover.id,sequence,barrel,peerId:shooterId,start:start.toArray(),end:point.toArray(),targetId:victim?.id??null,kind};
  ...
  const api={vehicles,current,vehicleHit,hit,
    ...
    attach(p){
      ...
      p.nav.vehicle={step:(dt,pad)=>current(p.id)?true:previous?.step?.(dt,pad)??false,
        ...
        constrainWalker:(a,b)=>{let end=previous?.constrainWalker?.(a,b)??b;for(const r of vehicles.values())end=constrainSentryWalker(r,a,end,p.id);return end;}};
    },
