...
| `npm run test:browser -- -c scripts/promenade-profile.config.js` | Retail promenade diagnostics, not gates: fixed-pose captures of the COSMIC CHICKEN prints with plane-to-anchor distances, and an in-page per-frame walk profile that attributes stalls to the WebGL calls that block (`PROMENADE_ANGLE=d3d11` for a Windows laptop's backend) |
