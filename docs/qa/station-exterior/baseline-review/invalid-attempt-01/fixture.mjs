...
test('independent full station exterior baseline',async({page,browser})=>{
 ...
 try{
  for(const url of ['/src/station-architecture.js','/src/station-complex.js','/models/station.glb','/models/station_lod1.glb']){const r=await page.request.get(url);expect(r.ok()).toBe(true);const bytes=await r.body();report.served[url]={bytes:bytes.length,sha256:crypto.createHash('sha256').update(bytes).digest('hex')};}
  await page.goto('/?intro=0&seed=7291&debug=1');await page.waitForFunction(()=>window.starAgent?.state.ready&&window.starAgent.state.station.ready&&window.starAgent.state.station.finish==='ready',null,{timeout:90000});
