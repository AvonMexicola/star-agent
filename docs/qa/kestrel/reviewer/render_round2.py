...
OUT=ROOT/'docs/qa/kestrel/reviewer/round-2'
...
scene.world.node_tree.nodes.get('Background').inputs['Color'].default_value=(.07,.07,.07,1)
...
def material(name, color, emit=False):
    ...
    return m
clay=material('REVIEWER clay',(.36,.36,.36))
for o in asset:o.data.materials.clear();o.data.materials.append(clay)
for name,pos,power,size in [('REVIEWER Key',(-7,11,-9),2500,8),('REVIEWER Fill',(8,7,4),1400,7)]:
