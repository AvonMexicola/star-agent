...
`/dev/kestrel.html`. The asset-stage record below describes the reviewed GLB.
The separate [flight integration](kestrel-flight.md) adds fleet selection, live
flight and physical ladder access. Suspension forces and combat remain outside
that integration.
...
entered `public/models/`; Vite bundles the exact reviewed GLB for the studio and
the separate gameplay integration.
