...
than silently approximated. The S2 dimensions are a Star Agent fitting allocation,
not a claim about another game’s weapon dimensions.
...
Kestrel declares four empty fixed S2 sockets: `HP_Nose`, `HP_WingL`, `HP_WingR`,
and `HP_Belly`. Its underside mating frames face outward (+Y down in ship space)
while preserving −Z forward. Existing protective cover geometry remains intact;
this socket assignment does not install guns or certify a future weapon package’s
clearance. The original geometry, materials and mechanism animations are retained.

Nomad’s active utility revision declares two S1 sockets, `HP_Weapon_Port` and
`HP_Weapon_Starboard`. Its actual animated fit is reviewed in that separate PR.
Atlas Mark II retains its three S3 slots. `NOMAD_FUTURE_S1_ATTACHMENT` remains a
legacy debug-gauge export for existing callers.
