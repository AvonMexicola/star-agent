"""Original deterministic PBR swatches for Meridian's manufactured gun kit.

Run with system Python/Pillow before build_ship_weapons.py. These are generated
...
OUT = ROOT / 'assets/ship-weapons/textures'
...
for y in range(EDGE):
    for x in range(EDGE):
        ...
        scuff = (1 if (x * 17 + y * 3) % 487 < 2 and .16 < v < .82 else 0)
        edge = max(0, 1 - min(u, 1-u, v, 1-v) * 22)
        factor = 1 + grain * .018 + brush * .018 - edge * .035 + scuff * .045
...
(OUT / 'provenance.json').write_text(json.dumps({
    ...
    'builder': 'blender/ship_weapon_textures.py', 'seed': 7291, 'size': [EDGE, EDGE],
    'baseColorSpace': 'sRGB', 'dataMaps': 'linear ORM and tangent-space normal',
    'files': records}, indent=2) + '\n')
print('Generated original weapon PBR maps.')
