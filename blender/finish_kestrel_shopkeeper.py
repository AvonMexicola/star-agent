"""Kestrel merchant: shared skin/geometry, four preserved Meshy idle clips.

ALSOFT_DRIVERS=null blender -b --python-exit-code 1 \
  --python blender/finish_kestrel_shopkeeper.py -- [--skip-renders]

Derived from female exporter snapshot SHA256 17f91305242256a044fe91fe471c54388492cc97e6fdb68aa394e7120602181f.
Standalone Blender authoring/validation; no hosted services or runtime edits.
"""
import argparse,copy,hashlib,json,math,statistics,struct,subprocess,sys,zipfile,bisect
...
PACK=ROOT/'assets/characters/kestrel-shopkeeper'
OUT=ROOT/'public/models/characters/kestrel-shopkeeper.glb'
ARCHIVE_SHA='549dc3627433c39238861af916d2cda52f44e92a0367f7ab5c69a7e5d33adc78'
LABELS=['02','03','11','12']
...
def ensure_sources():
    archive=PACK/'source/source-animation-pack.zip'
    assert identity(archive)['sha256']==ARCHIVE_SHA
    receipt=json.loads((PACK/'source/archive-manifest.json').read_text())
    with zipfile.ZipFile(archive) as z:
        for entry in receipt['entries']:
            if not entry['includedRuntime']:continue
            path=PACK/'source'/Path(entry['archivePath']).name
            if not path.exists():path.write_bytes(z.read(entry['archivePath']))
            assert identity(path)['sha256']==entry['sha256'] and path.stat().st_size==entry['bytes']


def combined_source():
    ensure_sources()
    files=[PACK/f'source/Meshy_AI_Cybertech_Mechanic_biped_Animation_Idle_{label}_withSkin.glb'for label in LABELS]
    base,raw=read_glb(files[0]);binary=bytearray(raw);base=copy.deepcopy(base)
    original=copy.deepcopy(base);base['animations'][0]['name']='idle-02'
    receipts=[]
...
def optimize(mesh,doc,binary):
    ...
    protected=[]
    # Keep multi-joint boundaries intact; merging them can create >4 substantial influences.
    joint_sets=[{g.group for g in v.groups if g.weight>.01} for v in mesh.data.vertices]
    boundary=set()
    for edge in mesh.data.edges:
        a,b=edge.vertices
        if len(joint_sets[a]|joint_sets[b])>4:boundary.update((a,b))
    for v in mesh.data.vertices:
        weight=sum(g.weight for g in v.groups if group_names[g.group] in ['Head','head_end','headfront','LeftHand','RightHand'])
        if weight>.35 or v.index in boundary:protect.add([v.index],1,'REPLACE');protected.append(v.index)
    ...
    modifier.ratio=19700/31252;modifier.use_collapse_triangulate=True
    ...
    report={'sourceTriangles':31252,'runtimeTriangles':triangles,'runtimeVertices':len(unique),'protectedSourceVertices':len(protected),'sourceBlenderCorrespondenceMax':float(residual),'restSurfaceErrorLocalMax':max(errors),'restSurfaceErrorLocalP95':float(np.percentile(errors,95)),'protectedRestSurfaceErrorLocalMax':max(protected_errors),'maxDiscardedSkinWeight':max(discard),'uvVFlipForGltf':flip_uv,'sourceToBlenderMesh':matrix.tolist()}
    assert max(discard)<.01,report
...
def relaxed_arm_tracks(doc,binary,clips):
    ...
    for clip in clips:
        ...
        for time in times:
            ...
            for side,sign in [('Left',1),('Right',-1)]:
                ...
                for bone,child,direction in [(shoulder,upper,(sign,-.075,.16)),(upper,fore,(sign*.12,-1,.075)),(fore,hand,(sign*.045,-1,.29))]:
                ...
                # Calibrate the supplied hand's dorsal direction once, then align each
                # clip's varying wrist twist independently. Axial roll cannot move wrists.
                axis=(world(hand).translation-world(fore).translation).normalized()
                handq=world(hand).to_quaternion()
                if side not in hand_back_local:
                    normal=chest@Vector((0,0,1));normal=(normal-axis*normal.dot(axis)).normalized()
                    hand_back_local[side]=handq.inverted()@normal
                current=handq@hand_back_local[side];current=(current-axis*current.dot(axis)).normalized()
                desired=chest@Vector((sign,0,0));desired=(desired-axis*desired.dot(axis)).normalized()
                angle=math.atan2(axis.dot(current.cross(desired)),current.dot(desired))
                rotations[fore]=rotations[fore]@Quaternion((0,1,0),angle*strength)
    ...
    (PACK/'arm-correction.json').write_text(json.dumps({'method':'Directional shoulder/upperarm/forearm retarget, 30 Hz, original bind/node transforms preserved; source high gestures retain original rotations as correction fades between hand-relative heights -0.28 and -0.08 m.','sourceForward':'+Z','restForearmAxialRoll':'Per-frame signed axial palm alignment toward thighs; calibrated dorsal vector from corrected idle02 start, retains gesture fade','targetDirectionsChestSpace':{'shoulder':['side',-.075,.16],'upperArm':['side*0.12',-1,.075],'forearm':['side*0.045',-1,.29]},'clips':receipts},indent=2)+'\n')
    return receipts
...
def replace_geometry(doc,binary,attributes,indices):
    ...
    for m in doc['materials']:
        ...
        m['name']='Kestrel mechanic / source albedo / matte non-emissive'
    result=bytearray()
...
def preservation_receipt(doc,binary,clips):
    receipt=[]
    for label in LABELS:
        source=PACK/f'source/Meshy_AI_Cybertech_Mechanic_biped_Animation_Idle_{label}_withSkin.glb';original,raw=read_glb(source)
        ...
        receipt.append({'name':exported['name'],'originalChannelsUnchanged':sum(t['status']=='byte-identical'for t in tracks),'correctedChannels':sum(t['status']!='byte-identical'for t in tracks),'addedChannels':['Kestrel_normalized_root.translation'],'inverseBindSha256':hashlib.sha256(a).hexdigest(),'tracks':tracks})
...
def crossfade_report(arm,mesh,clips):
    ...
    activate('idle-02');frame(0);return [{k:v for k,v in r.items()if k!='samples'}for r in reports]


def studio(arm,mesh,clips,preview=False,only=None,prefix=""):
    ...
    shots=[('front-oblique','idle-02',0,(2,5,1),False),('back-oblique','idle-02',0,(-2,-5,1),False),('face-detail','idle-02',0,(1,5,.25),True)]
    if not preview:shots.extend((c['name']+'-mid',c['name'],c['duration']*.5,(2,5,1),False)for c in clips)
    for label,name,time,direction,close in shots:
        if only and label not in only:continue
        activate(name);frame(time);target=Vector((0,0,1.61 if close else .9));cam.data.ortho_scale=.58 if close else 2.2;cam.location=target+Vector(direction);cam.rotation_euler=(target-cam.location).to_track_quat('-Z','Y').to_euler()
        scene.render.stamp_note_text=f'BLENDER CPU STUDIO / {prefix}{name} {time:.2f}s / NOT GAME';png=review/(prefix+label+'.png');scene.render.filepath=str(png);bpy.ops.render.render(write_still=True)
        subprocess.run(['magick',str(png),'-quality','91',str(review/(prefix+label+'.webp'))],check=True);png.unlink()


def main():
    parser=argparse.ArgumentParser();parser.add_argument('--skip-renders',action='store_true');parser.add_argument('--render-only',action='store_true');parser.add_argument('--pose-preview',action='store_true');args=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--'in sys.argv else [])
    assert identity(PACK/'source/source-animation-pack.zip')['sha256']==ARCHIVE_SHA
    if args.render_only:
        manifest=json.loads(OUT.with_name('kestrel-shopkeeper-manifest.json').read_text());assert identity(OUT)==manifest['asset']
        arm,mesh=load_scene(OUT);studio(arm,mesh,manifest['idleClips'],args.pose_preview);return
    doc,binary,clips=combined_source();combined=PACK/'source/combined-idles.glb';write_glb(combined,doc,binary)
    arm,mesh=load_scene(combined);activate('idle-02')
    ...
    binary=replace_geometry(doc,binary,attributes,indices);arm_correction=relaxed_arm_tracks(doc,binary,clips);derived=PACK/'derived-unfitted.glb';write_glb(derived,doc,binary);arm,mesh=load_scene(derived);activate('idle-02');frame(0)
    first=points(mesh);b=bounds(first);height=b['max'][1]-b['min'][1];factor=1.8/height;cx=(b['min'][0]+b['max'][0])/2;cz=(b['min'][2]+b['max'][2])/2
    # Source faces +Z. A rigid Y half turn makes the runtime face -Z.
    wrapper=len(doc['nodes']);doc['nodes'].append({'name':'Kestrel_normalized_root','children':doc['scenes'][doc.get('scene',0)]['nodes'],'rotation':[0,1,0,0],'scale':[factor]*3,'translation':[factor*cx,-factor*b['min'][1],factor*cz]});doc['scenes'][doc.get('scene',0)]['nodes']=[wrapper]
    grounding=[]
    for clip in clips:
    ...
    manifest={'id':'kestrel-shopkeeper','displayName':'Kestrel shopkeeper','asset':identity(OUT),'sourceArchive':identity(PACK/'source/source-animation-pack.zip'),'idleClips':clips,'dimensions':dimensions,'boundsMetres':envelope,'triangles':optimization['runtimeTriangles'],'vertices':optimization['runtimeVertices'],'materials':1,'skins':1,'joints':24,'texture':{'format':'WebP','maximumEdge':1024,'sourceAlbedoPreserved':True},'material':{'emission':0,'metallic':0,'roughness':.72,'reason':'Remove source full-albedo self-light and default-metal error; matte mixed workwear finish'},'normalization':{'anatomicalLandmark':'Idle-02 initial head/hair crown over boot soles','targetHeightMetres':1.8,'scale':factor,'facing':'-Z','up':'+Y','sourceCenterXZ':[cx,cz],'rootTravelRemoved':False,'reason':'Idle clips retain authored bounded horizontal weight shifts; wrapper X/Z remain constant and only vertical sole grounding is added.'},'armCorrection':arm_correction,'grounding':grounding,'optimization':optimization,'validation':{'kind':'actual runtime GLB Blender reimport, CPU skin evaluation','samplesHz':60,'clips':[{k:v for k,v in r.items()if k!='samples'}for r in reports],'gameEvidence':False,'preservation':preservation,'crossfades':transitions}}
    OUT.with_name('kestrel-shopkeeper-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
    activate('idle-02');frame(0);bpy.context.preferences.filepaths.save_version=0
    bpy.ops.wm.save_as_mainfile(filepath=str(PACK/'kestrel-shopkeeper.blend'))
