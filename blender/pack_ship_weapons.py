"""Pack original gun textures as WebP and finalize actual exported asset counts."""
...
path=ROOT/'public/models/ship-weapons.glb'
...
manifest_path=ROOT/'assets/ship-weapons/manifest.json'
m=json.loads(manifest_path.read_text())
for variant in m['variants']:
    meshes=[mesh for mesh in j['meshes'] if mesh.get('name','').startswith(variant['id']+'_')]
    variant['triangles']=sum(j['accessors'][p['indices']]['count']//3 for mesh in meshes for p in mesh['primitives'])
for adapter in m.get('adapters',[]):
    meshes=[mesh for mesh in j['meshes'] if mesh.get('name','').startswith(adapter['root']+'_')]
    adapter['triangles']=sum(j['accessors'][p['indices']]['count']//3 for mesh in meshes for p in mesh['primitives'])
m.update({'asset':'public/models/ship-weapons.glb','sha256':hashlib.sha256(data).hexdigest(),
          'bytes':len(data),'triangles':sum(j['accessors'][p['indices']]['count']//3 for mesh in j['meshes'] for p in mesh['primitives']),
          'packing':packing,'textureFormat':'WebP, 1024x1024, original shared PBR swatches',
