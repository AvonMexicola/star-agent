...
const out=process.env.TERMINAL_EVIDENCE??'/tmp/star-agent-projected-terminals-01';
...
test('projected terminal connects to its exchange dashboard through a complete controller cargo journey',async({page,browser})=>{
 ...
 const terminal=await page.evaluate(()=>window.starAgent.state.settlements.sites.find(s=>s.body==='selene').terminal);await aim(page,terminal,'world');
 await page.waitForFunction(()=>window.starAgent.state.trading.projections.ids.includes('settlement-selene'));
 const projections=await page.evaluate(()=>window.starAgent.state.trading.projections);expect(projections.count).toBeLessThanOrEqual(6);expect(projections.draws).toBeLessThanOrEqual(12);
 await capture(page,'exchange-entrance');
 ...
 await button(7,true);await page.evaluate(()=>window.settlementPad.mapping='');await frames(page);expect(await page.evaluate(()=>window.starAgent.state.controller.armed)).toBe(false);await page.keyboard.press('Escape');await expect(page.locator('dialog[open]')).toBeVisible();await page.keyboard.press('Escape');await page.waitForFunction(()=>window.starAgent.state.enabled&&!document.querySelector('dialog[open]'));await page.evaluate(()=>window.settlementPad.mapping='standard');await frames(page);expect(await page.evaluate(()=>window.starAgent.state.controller.armed)).toBe(false);await button(7,false);await page.waitForFunction(()=>window.starAgent.state.controller.armed);
...
test('owner offer and beacon controls stay usable in the desktop and phone dashboard',async({page})=>{
 ...
 await page.locator('[data-controller-key="base-offer-add"]').tap();await expect(page.locator('.base-stock-row')).toContainText('For sale: 2 SBU');await page.screenshot({path:out+'/owner-phone.png'});await page.locator('[data-controller-key="base-open"]').tap();await expect(page.locator('[data-controller-key="base-open"]')).toHaveText('Shop: Closed');
