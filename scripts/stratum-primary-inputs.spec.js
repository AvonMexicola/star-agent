...
const sourceFiles = ['src/main.js', 'src/navigation.js', 'src/medium-ships.js', 'src/medium-ship-lights.js', 'src/medium-ship-gameplay.js',
  ...
  'src/flight-model.js', 'src/ship-handling.js', 'src/mining/field.js', 'src/mining/rock.js', 'src/mining/store.js',
...
test('Stratum: real landing and ramp → short flight → saved twin ore → native inventory and return', async ({page, browser}, testInfo) => {
  ...
  const errors = [], warnings = [], requests = [], milestones = [], flight = [], accessPaths = {}, gates = {}, sightlines = [], persistence = {};
  ...
  async function sightline(name) {
    const record = await page.evaluate(displays => {
      ...
      const camera = n.position.clone().fromArray(s.camera.position), inverse = n.orientation.clone().invert();
      ...
      return {eye: n.position.toArray(), camera: s.camera, orientation: n.orientation.toArray(), shipOrientation: q.toArray(), mfds: s.mfds,
        ...
        }), projectionLimit: 'Canonical display centers projected with actual FOV/origin and navigation attitude; this is not a triangle/occlusion or readability certificate'};
    }, S.displays);
  ...
  async function saved() {
    const raw = await page.evaluate(key => localStorage.getItem(key), MINING_KEY); expect(raw).not.toBeNull();
    const store = new MiningStore({getItem: key => key === MINING_KEY ? raw : null, setItem() { throw Error('Read-only saved-state probe'); }});
    expect(store.blocked).not.toBe(true); expect(store.warning).toBe('');
    return {sha256: hash(raw), revision: store.state.revision,
      rocks: Object.fromEntries(Object.entries(store.state.rocks).map(([id, rock]) => [id, rock.revision])),
      ore: store.container('stratum-ore'), pack: store.container('pack'), supplies: store.container('ship'), limit: store.limits('stratum-ore').resources};
  }
  ...
  try {
    ...
    persistence.target = {id: s.mining.activeRock, position: target, revision: s.mining.activeRevision};
    ...
    phase = 'native-twin-mining'; const before = await state(page); persistence.before = await saved();
    ...
    persistence.beams = live.mining.ship.beams; await shot('04-native-twin-cutters'); await page.waitForTimeout(1500); await input.hold([]);
    await wait(page, () => starAgent.state.mining.ship.beaming === 0 && !starAgent.state.mining.pending); persistence.mined = await saved();
    expect(persistence.mined.limit).toBe(384); expect(mass(persistence.mined.ore.items)).toBeGreaterThan(mass(persistence.before.ore.items) + .02);
    expect(persistence.mined.pack.items).toEqual(persistence.before.pack.items); expect(persistence.mined.supplies.items).toEqual(persistence.before.supplies.items);
    const hit = live.mining.ship.hitIds[0]; expect(hit === ROCK_ID ? persistence.mined.revision > persistence.before.revision : (persistence.mined.rocks[hit] ?? 0) > (persistence.before.rocks[hit] ?? 0)).toBe(true);
    if (!phone) {
    ...
    await note('Twin real hits commit voxel cuts and persistent Stratum ore');
    phase = 'native-ore-dialog-and-neutral'; await cargoGate(); await note('Visible ore transfer conserves the real saved containers');
    phase = 'native-tab-focus-neutral'; await nativeFocusNeutral(page, input, gates.focus = {});
    phase = 'actual-pilot-control-return'; await input.reset(); await brake(); await wait(page, () => !starAgent.state.mining.pending); persistence.final = await saved();
    ...
    await writeFile(dir + '/journey.json', JSON.stringify({result: complete ? 'PASS' : 'FAIL', phase, failed, mode: testInfo.project.name,
      ...
      milestones, accessPaths, flight, gates, persistence, sightlines, inputLog: input.log, diagnostics, final: await state(page).catch(() => null),
      limits: ['No physical keyboard/controller/touchscreen hardware claim', 'Saved bytes are re-read without a gameplay reload',
