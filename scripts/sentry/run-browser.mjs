...
import {stopOwnedSentryProcesses} from './owned-processes.mjs';
...
const sourceFiles=[...(await readdir(path.join(root,'src/sentry'))).filter(n=>/\.(js|css)$/.test(n)).map(n=>'src/sentry/'+n),'src/rover-carrier.js','src/rover-physics.js','src/rover-support.js','src/multiplayer/client.js','src/multiplayer/remote-players.js','src/inventory/ship-access.js','src/combat/ship-occupancy.js','scripts/sentry/run-browser.mjs','scripts/sentry/owned-processes.mjs','scripts/sentry/sentry.config.js','scripts/sentry/sentry.spec.js','scripts/sentry/eva-feedback.mjs','src/main.js','src/multiplayer/protocol.js','server/sentry.js','server/room.js','server/index.js','server/world.js','src/navigation.js','src/navigation-rotation.js','src/planet-rotation.js','src/planet-render-frames.js','server/security.js','server/combat.js','public/models/burrow-sentry.glb'];
sourceFiles.push('scripts/sentry/access-evidence.mjs');
...
const cleanup=await stopOwnedSentryProcesses(root,output);await writeFile(path.join(output,'cleanup.json'),JSON.stringify(cleanup,null,2));if(cleanup.after.length)code=code||1;
const after=await hashes(),changed=sourceFiles.filter(name=>before[name]!==after[name]);
await writeFile(path.join(output,'receipt.json'),JSON.stringify({completed:new Date().toISOString(),code,signal,changed,before,after,build:buildReceipt,cleanup},null,2));
