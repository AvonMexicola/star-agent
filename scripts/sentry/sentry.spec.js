...
async function register(page,index,errors){
  await pad(page);await boot(page,'/?dev=1&intro=0&start=hangar&debug=1',errors);
...
test('two real connected players reach one rover physically and hand gunner authority back to its driving pilot',async({browser})=>{
  const a=await browser.newContext({viewport:{width:1280,height:800}}),b=await browser.newContext({viewport:{width:1280,height:800}}),pilot=await a.newPage(),gunner=await b.newPage(),errors=[],report={input:'Desktop account text; then injected standard Gamepad for deploy, physical station/EVA approach, both seats, drive/aim/fire/backpack/exit. No physical pad or debug pose writes.'};
  try{
    await register(pilot,1,errors);await register(gunner,2,errors);
    ...
    await walk(pilot,await roverPoint(pilot,[-2.5,1.75,-.1],id));report.pilotAccess=await board(pilot,'pilot');
    ...
    await walk(gunner,await podPoint(gunner,own,{x:10,z:-12}),{timeout:60000});await wait(gunner,()=>starAgent.state.mode==='eva');
    await walk(gunner,await podPoint(gunner,own,{x:10,y:9,z:-45}),{eva:true,timeout:90000});
    await walk(gunner,await podPoint(gunner,hangar,{x:10,y:9,z:-45}),{eva:true,timeout:90000});
    ...
    await walk(gunner,await roverPoint(gunner,[2.1,1.75,3.45],id));await walk(gunner,await roverPoint(gunner,[0,1.75,3.45],id));report.gunnerAccess=await board(gunner,'gunner');
    ...
    report.fallback=(await state(pilot)).sentry.current;await neutral(pilot);await tap(pilot,8);await expect(pilot.locator('#multiplayer-inventory-dialog')).toBeVisible();await tap(pilot,1);await neutral(pilot);
    await tap(pilot,2);await wait(pilot,()=>!starAgent.state.sentry.occupied,null,45000);expect([...errors]).toEqual([]);
  }finally{report.errors=[...errors];report.warnings=errors.warnings;report.requests=errors.requests;report.renderer=await rendererInfo(pilot).catch(()=>null);report.pilot=await state(pilot).catch(()=>null);report.gunner=await state(gunner).catch(()=>null);await writeFile(output+'/multiplayer-report.json',JSON.stringify(report,null,2));await a.close();await b.close();}
});
...
test('keyboard and native phone controls board, aim, fire and leave the Sentry',async({browser})=>{
  ...
  for(const mobile of [false,true]){
    const context=await browser.newContext({viewport:mobile?{width:390,height:844}:{width:1440,height:900},isMobile:mobile,hasTouch:mobile}),page=await context.newPage();
    try{
