...
test('two real connected players reach one rover physically and hand gunner authority back to its driving pilot',async({browser})=>{
  ...
  try{
    ...
    await tap(pilot,2);await wait(pilot,()=>!starAgent.state.sentry.occupied,null,45000);expect((await state(pilot)).multiplayer.connected).toBe(true);expect([...errors]).toEqual([]);console.log('Sentry MP: server inventory and physical return complete');
