...
for(const body of ['aeon','selene','pyre','miasma'])test(`corporate buildings and pad identity on ${body}`,async({page,browser})=>{
 const record=await setup(page,false);await page.goto(`/?dev=1&ship=nomad&start=settlement-${body}&intro=0&debug&seed=7291&epoch=1788876000000`);await page.waitForFunction(()=>window.starAgent?.state.ready&&window.starAgent.state.enabled&&!window.starAgent.state.transiting&&window.starAgent.state.settlements.ready&&window.starAgent.state.settlements.rendered>0,undefined,{timeout:90000});
 ...
 for(const [name,eye,look] of [['approach',[70,42,92],[0,2,12]],['entrance',[8,2,-5],[2,1.5,-16]]]){
  // Art-only camera placement after the developer transit releases navigation.
  // Full controller construction is checked separately below.
  const expectedPosition=await page.evaluate(({body,eye,look})=>{const n=window.starAgent.navigation,s=window.starAgent.state.settlements.sites.find(s=>s.body===body),q=n.orientation.clone().fromArray(s.quaternion),o=n.position.clone().fromArray(s.origin),deck=n.position.clone().fromArray(s.pad).sub(o).applyQuaternion(q.clone().invert()).y,world=a=>n.position.clone().fromArray(a).add(n.position.clone().set(0,deck,0)).applyQuaternion(q).add(o);n.mode='walk';n.insideShip=false;n.enabled=false;n.position.copy(world(eye));n.orientToward(world(look),n.normal);n.velocity.set(0,0,0);return n.position.toArray();},{body,eye,look});
  await page.waitForFunction(()=>{const s=window.starAgent.state;return s.body==='aeon'?s.terrainLod.settled:s.body==='selene'?s.moon.pending===0&&s.moon.effects.settled:s[s.body].pending===0&&s[s.body].morphing===0;},undefined,{timeout:60000});await frames(page);expect(await page.evaluate(expected=>{const n=window.starAgent.navigation;return !n.enabled&&!window.starAgent.state.transiting&&n.position.distanceTo(n.position.clone().fromArray(expected))<.001;},expectedPosition)).toBe(true);await capture(page,`${body}-${name}`);
 }
