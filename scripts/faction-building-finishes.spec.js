...
test('controller enters sandbox, chooses paint and print, builds, repaints, persists and returns',async({page,browser})=>{
 ...
 await page.waitForFunction(()=>window.starAgent.state.build.pieceCount===11);await tap(page,2);await ready(page);await aim(page,await pointForWall(page));await capture(page,'wall-before-repaint');
 ...
 expect((await saved(page)).build.claims[0].pieces.find(p=>p.type==='wall').finish).toBe('petrol');await tap(page,0);await tap(page,2);await ready(page);await capture(page,'helmet-painted');
...
for(const body of ['aeon','selene','pyre','miasma'])test(`corporate buildings and pad identity on ${body}`,async({page,browser})=>{
 ...
 expect(record.errors).toEqual([]);await writeFile(`${out}/${body}-art.json`,JSON.stringify({browser:browser.version(),...await diagnostics(page),...record,fixture:'Fixed art camera only; corporate exterior/pad sign review, no traversal claim'},null,2));
});
