...
test('Selene landing, ramp exploration, lunar jump, reboarding and launch render correctly',async({page,browser})=>{
  ...
  expect(outside.moon.effects.ringAsteroids).toBe(20_971_520);expect(outside.moon.effects.iceParticles).toBeGreaterThan(0);expect(outside.insideShip).toBe(false);expect(outside.altitude).toBeCloseTo(1.75,5);
