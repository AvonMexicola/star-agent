...
test.describe('native phone and keyboard',()=>{
 ...
 test('Hush actual airlock, service and trade handlers fit a native phone',async({page,browser})=>{
  ...
  expect(errors).toEqual([]);await writeFile(`${out}/hush-native-phone.json`,JSON.stringify({browser:browser.version(),graphics:await graphics(page),errors,warnings,doors,controlledPose:true,physicalController:false},null,2));
 });
});
...
test('Miasma weaker turret and both Crimson vacuum habitats render',async({page,browser})=>{
 ...
 for(const site of ['pirate-veil','pirate-hush']){
  ...
  for(const [name,eye,look] of [['overview',[48,28,61],[0,5,-10]],['crimson-work-yard',[-2,3,24],[-8,1,8]],['airlock-approach',[12,2,7],[6,1.5,-5]],['sealed-exchange',[11,1.8,-17],[6,1.5,-27]],['outer-apron',[34,20,296],[0,1,260]]]){
   ...
   await page.waitForFunction(()=>{const s=window.starAgent.state;return s.pirateCompound.site.body==='miasma'?s.miasma.ready&&s.miasma.pending===0:s.moon.pending===0&&s.moon.effects.settled;},undefined,{timeout:60000});await frames(page);await page.addStyleTag({content:'body > :not(canvas) { visibility: hidden !important; }'});await capture(page,`${site.slice(7)}-${name}`);
  }
  ...
  expect(errors).toEqual([]);receipts.push({site,browser:browser.version(),graphics:await graphics(page),errors,warnings,balance,performance,fixture:true,state:await page.evaluate(()=>window.starAgent.state.pirateCompound)});
 }
