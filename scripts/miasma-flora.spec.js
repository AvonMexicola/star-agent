...
test('thin toxic atmosphere and Claude flora render with stable placement and wind',async({page,browser})=>{
 ...
 await page.goto('/?intro=0&debug');await page.waitForFunction(()=>window.starAgent?.state.ready);await page.keyboard.press('Shift+Tab');
 ...
 await page.keyboard.press('Shift+Tab');await expect(page.locator('#toxic-warning')).toBeVisible();await shot('grove-hud');
 ...
 await page.waitForFunction(()=>window.starAgent.state.pyre.ready);await page.keyboard.press('Shift+Tab');await shot('pyre-companion');
