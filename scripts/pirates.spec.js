...
test('keyboard entry and physical exit, native phone crouch while walking',async({page,browser})=>{
 ...
 await page.keyboard.down('w');await page.waitForFunction(()=>window.starAgent.state.shipLocal[2]>2.6,undefined,{timeout:30000});await page.keyboard.up('w');expect((await state(page)).interaction).toMatch(/HATCH/);await page.keyboard.press('f');await page.waitForFunction(()=>window.starAgent.state.doorProgress===1);
 await page.keyboard.down('w');await page.waitForFunction(()=>!window.starAgent.state.insideShip&&window.starAgent.state.shipLocal[2]>13,undefined,{timeout:45000});await page.keyboard.up('w');
