...
const tap=async(page,index)=>{await setButton(page,index,true);await page.waitForTimeout(180);await setButton(page,index,false);await page.waitForTimeout(180);};
...
async function aimAtDeposit(page) {
  for(let i=0;i<120;i++) {
    const error=await page.evaluate(()=>{
      ...
      const local=n.position.clone().fromArray(window.starAgent.state.mining.position).sub(n.position).applyQuaternion(n.orientation.clone().invert());
...
test('controller alone transits, lands, walks out, aims, equips, mines with a visible beam and opens backpack',async({page,browser})=>{
  ...
  for(let i=0;i<24;i++) {if(await page.locator('[data-controller-key="destination-moon"]').evaluate(el=>el===document.activeElement))break;await tap(page,13);}
  await expect(page.locator('[data-controller-key="destination-moon"]')).toBeFocused();await tap(page,0);
  ...
  await axes(page,[0,-1,0,0]);await page.waitForFunction(()=>{const n=window.starAgent.navigation;return n.position.distanceTo(n.position.clone().fromArray(window.starAgent.state.mining.position))<5;});await axes(page,[0,0,0,0]);await aimAtDeposit(page);
  ...
  const revision=await page.evaluate(()=>window.starAgent.state.mining.revision);
  await setButton(page,7,true);await page.waitForFunction(previous=>window.starAgent.state.mining.revision>previous,revision);
  await page.waitForFunction(()=>window.starAgent.state.mining.tool.beaming);
  await page.screenshot({path:`${evidence}/controller-held-beam.png`});
  ...
  await tap(page,8);await expect(page.locator('#cargo-dialog')).toBeVisible();await page.screenshot({path:`${evidence}/controller-backpack.png`});
  ...
  await writeFile(`${evidence}/controller-gameplay.json`,JSON.stringify({browser:browser.version(),input:'Injected W3C standard Gamepad; no physical controller used',viewport:page.viewportSize(),mined,errors},null,2));
