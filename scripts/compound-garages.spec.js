...
test('controller lands, walks to garage, deploys and physically boards Burrow, drives to terrain, opens ore and returns',async({page,browser})=>{
 ...
 expect(await page.evaluate(()=>window.starAgent.state.position)).toEqual(pose);
 ...
 await tap(2);await page.waitForFunction(()=>!window.starAgent.state.rover.occupied&&!window.starAgent.state.rover.busy,undefined,{timeout:25000});await capture(page,'return-to-surface');
 ...
 await page.locator('[data-controller-key="garage-retrieve"]').tap();await expect(page.locator('.garage-status')).toContainText('Burrow is ready');await page.locator('[data-controller-key="garage-close"]').tap();await page.waitForFunction(()=>window.starAgent.state.enabled);
...
for(const body of ['aeon','selene','pyre','miasma'])test(`garage renders on ${body}`,async({page,browser})=>{
 ...
 await page.addStyleTag({content:'body > :not(canvas){visibility:hidden!important}'});await page.waitForFunction(()=>{const s=window.starAgent.state;return s.body==='aeon'?s.terrainLod.settled:s.body==='selene'?s.moon.pending===0&&s.moon.effects.settled:s[s.body].pending===0&&s[s.body].morphing===0;},undefined,{timeout:60000});await page.waitForTimeout(1000);await capture(page,`${body}-compound`);expect(errors).toEqual([]);await writeFile(`${out}/${body}-art.json`,JSON.stringify({browser:browser.version(),graphics:await graphics(page),errors,warnings,artPoseOnly:true},null,2));
});
