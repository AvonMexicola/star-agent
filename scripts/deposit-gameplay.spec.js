...
for(const input of ['controller','keyboard','touch','full-ship'])test(`physical ship Deposit all: ${input}`,async({page,browser})=>{
  ...
  let initial=store.withItems({...store.state,starterConstruction:{version:1,claimed:true}},'pack',pack);initial=store.withItems(initial,'ship',{...emptyItems(),basalt:input==='full-ship'?store.limits('ship',initial).resources:4});expect(store.validContainers(initial)).toBe(true);expect(store.write(initial)).toBe(true);
