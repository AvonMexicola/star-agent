...
test('dedicated multiplayer retains account entry and neutral controller flow with scene tools enabled',async({page,browser})=>{
 ...
 await page.goto('http://127.0.0.1:5691/?seed=7291&debug');await page.waitForFunction(()=>window.starAgent?.state.ready,undefined,{timeout:120000});
 ...
 const metadata=await (await page.request.get('http://127.0.0.1:5691/release.json')).json();expect(metadata.multiplayerVersion).toBe(MULTIPLAYER_VERSION);expect(metadata.maxPlayers).toBe(MAX_PLAYERS);expect(metadata.devTools).toBe(true);
