...
for(const ship of ['nomad','kestrel'])test(`${ship}: controller patrol console, physical flight, targeting, engagement and combat report`,async({page,browser})=>{
 ...
 await tap(9);await button(0,true);await button(1,true);await button(1,false);await frames();
 ...
 await button(0,false);await page.waitForFunction(()=>window.starAgent.state.controller.armed);
 ...
 // Read-only pose feedback steers injected right-stick axes; A is the sole firing route.
 await page.evaluate(()=>{
  window.combatPilot=setInterval(()=>{
   ...
   if(!t||s.combat.phase!=='engage'){pad.axes=[0,0,0,0];pad.buttons[0]={pressed:false,value:0};return;}
...
test('keyboard and pointer fire, authored NPC close-ups, loss and recovery',async({page})=>{
 ...
 await page.setViewportSize({width:390,height:844});await page.locator('#patrol-console-button').click();await page.screenshot({path:`${output}/inspection/phone-console.png`});
