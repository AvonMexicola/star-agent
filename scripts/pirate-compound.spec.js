...
test('work yard diagnostic identifies the visible rectangle',async({page})=>{
 await setup(page);
 ...
 await page.waitForFunction(()=>window.starAgent.state.moon.pending===0&&window.starAgent.state.moon.effects.settled,undefined,{timeout:60000});await frames(page);await page.addStyleTag({content:'body > :not(canvas) { visibility: hidden !important; }'});await capture(page,'rectangle-baseline');
 const hits=await page.evaluate(()=>{
  const n=window.starAgent.navigation,s=window.starAgent.state,scene=n.station.scene,ray=n.position.clone().set((950/innerWidth*2-1)*Math.tan(s.camera.fov*Math.PI/360)*innerWidth/innerHeight,(1-550/innerHeight*2)*Math.tan(s.camera.fov*Math.PI/360),-1).normalize().applyQuaternion(n.orientation),hits=[];
  scene.updateMatrixWorld(true);
  scene.traverse(mesh=>{if(!mesh.isMesh)return;for(let p=mesh;p;p=p.parent)if(!p.visible)return;
   const g=mesh.geometry;if(!g.attributes.position)return;if(!g.boundingSphere)g.computeBoundingSphere();const matrices=[];if(mesh.isInstancedMesh){for(let i=0;i<mesh.count;i++){const m=mesh.matrixWorld.clone();mesh.getMatrixAt(i,m);matrices.push(m.premultiply(mesh.matrixWorld));}}else matrices.push(mesh.matrixWorld);
   for(const matrix of matrices){const sphere=g.boundingSphere.clone().applyMatrix4(matrix),projection=sphere.center.dot(ray);if(projection+sphere.radius<0||sphere.center.clone().addScaledVector(ray,-projection).length()>sphere.radius)continue;const p=g.attributes.position,idx=g.index,a=ray.clone(),b=ray.clone(),c=ray.clone();let nearest=Infinity;
    for(let i=0;i<(idx?.count??p.count);i+=3){a.fromBufferAttribute(p,idx?idx.getX(i):i).applyMatrix4(matrix);b.fromBufferAttribute(p,idx?idx.getX(i+1):i+1).applyMatrix4(matrix);c.fromBufferAttribute(p,idx?idx.getX(i+2):i+2).applyMatrix4(matrix);const e1=b.sub(a),e2=c.sub(a),h=ray.clone().cross(e2),det=e1.dot(h);if(Math.abs(det)<1e-9)continue;const inv=1/det,t=a.clone().negate(),u=t.dot(h)*inv;if(u<0||u>1)continue;const q=t.cross(e1),v=ray.dot(q)*inv;if(v<0||u+v>1)continue;const d=e2.dot(q)*inv;if(d>0)nearest=Math.min(nearest,d);}
    if(Number.isFinite(nearest)){const parents=[];for(let o=mesh;o;o=o.parent)parents.push(o.name||o.type);hits.push({distance:nearest,parents,material:[mesh.material].flat().map(m=>({name:m.name,type:m.type,color:m.color?.getHexString(),transparent:m.transparent,opacity:m.opacity})),bounds:sphere.radius});}
   }
  });return hits.sort((a,b)=>a.distance-b.distance).slice(0,20);
 });await writeFile(`${out}/rectangle-ray.json`,JSON.stringify(hits,null,2));
 await page.evaluate(()=>{for(const child of window.starAgent.navigation.station.scene.getObjectByName('Crimson salvage fixtures').children)child.visible=false;});await frames(page);await capture(page,'rectangle-props-hidden');
});
