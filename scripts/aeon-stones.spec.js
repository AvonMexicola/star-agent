...
test('actual Aeon stones render at walking and approach distances',async({page,browser},info)=>{
  ...
  for(const distance of [6,55,100]){
    ...
    await page.evaluate(({eye,target,up})=>{
      const a=window.starAgent,n=a.navigation;a.openingSequence?.leave();for(const dialog of document.querySelectorAll('dialog[open]'))dialog.close();a.setRenderScale(1);
...
test('controller travels, lands, walks to Aeon stone, mines and processes its feedstock',async({page,browser},info)=>{
  ...
  await page.goto('/?intro=0&debug=1&seed=7291');await ready(page);await frames(page);console.log('Controller world ready');
  ...
  for(let attempt=0;attempt<30;attempt++){
    const target=await page.evaluate(({d,attempt})=>{const n=window.starAgent.navigation;return n.position.clone().set((attempt%5-2)*.3,.45+Math.floor(attempt/5)%3*.2,0).applyQuaternion(n.orientation.clone().fromArray(d.quaternion)).add(n.position.clone().fromArray(d.position)).toArray();},{d,attempt});
  ...
  await page.screenshot({path:info.outputPath('controller-recipes.png')});await button(page,7,true);await tap(page,1);expect(await page.evaluate(()=>window.starAgent.state.controller.armed)).toBe(false);await button(page,7,false);await frames(page);
