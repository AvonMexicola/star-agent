...
test('swept lunar contact crashes on real terrain instead of the former 4 km boundary',t=>{
  ...
  near(bodyAltitude(nav.position,SELENE),3.2,1e-7);assert.equal(nav.crash.surface,'lunar regolith');assert.ok(nav.crash.impactSpeed>12);
});
