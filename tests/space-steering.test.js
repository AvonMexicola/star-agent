...
test('Xbox A/B buttons translate assisted space flight along ship up/down after rolling',t=>{
  const {nav,pad,reset}=setup(t);
  for(const attitude of ['survey','rolled'])for(const [button,sign] of [[0,1],[1,-1]]){
    ...
    assert.ok(movement.dot(shipUp)*sign>50,`${attitude}: button moves along ship-local vertical`);
...
test('assisted atmospheric vertical thrust retains radial ascent after rolling the ship',t=>{
  ...
  const radial=nav.normal,start=nav.position.clone();pad.buttons[0]={pressed:true,value:1};
  for(let frame=0;frame<20;frame++)nav.update(.025);
...
test('holding Xbox brake still allows yaw/pitch/roll while blocking thrust and inertial spin',t=>{
  ...
  for(const assist of [true,false]){
    ...
    pad.buttons[6]={pressed:true,value:1};pad.buttons[0]={pressed:true,value:1};pad.buttons[10]={pressed:true,value:1};pad.axes[1]=-1;pad.axes[2]=1;
    for(let frame=0;frame<20;frame++)nav.update(.025);
    const nose=forward.clone().applyQuaternion(nav.orientation).applyQuaternion(before.clone().invert());
    assert.ok(nose.x>.4,'braked ship yaws at normal manual rate');assert.ok(nav.position.equals(position),'brake suppresses held main thrust and vertical thrust');
