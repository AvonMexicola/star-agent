...
test('tractor validation reads the transaction ledger without publishing it before a successful commit', async () => {
  const fixture = await setup(), [player] = [...fixture.players.values()];
  player.nav.position.set(25_000_000_000, 10, 20); player.nav.orientation.identity();
  player.nav.mode = 'eva'; player.nav.insideShip = false; player.weapon = 'mining-laser-tool';
  const cached = structuredClone(fixture.trading.state);
  const current = (await fixture.store.transactCommerce(state => {
    state.markets[AEON_MARKET_ID].stock.basalt = 7;
    state.loose = { external: { id: 'external', resource: 'ice', sbu: 2,
      position: [25_000_000_000, 10, 17], quaternion: [0, 0, 0, 1], holder: null, until: 0, movedAt: 0 } };
    state.revision++; return { state };
  })).state;
  const message = { commandId: 'current-ledger-grab', revision: current.revision, op: 'tractor-grab', crate: 'external' };
  const commit = fixture.store.transactCommerce;
  fixture.store.transactCommerce = fn => commit(async state => {
    const result = await fn(state); assert.equal(result.state.loose.external.holder, player.id);
    throw Error('tractor COMMIT failure');
  });
  await assert.rejects(fixture.trading.request(player, message), /tractor COMMIT failure/);
  assert.deepEqual(fixture.trading.state, cached); assert.deepEqual(await fixture.store.loadCommerce(), current);
  fixture.store.transactCommerce = commit;
  await fixture.trading.request(player, message);
  const saved = await fixture.store.loadCommerce();
  assert.equal(saved.loose.external.holder, player.id); assert.equal(saved.markets[AEON_MARKET_ID].stock.basalt, 7);
  await fixture.trading.request(player, message);
  assert.deepEqual(await fixture.store.loadCommerce(), saved, 'replay cannot duplicate freight or refill the market');
});
