...
test('Crimson derivatives retain exact source geometry and three bounded native textures',()=>{
 ...
 for(const record of m.assets){const source=readFileSync(new URL('../'+record.source,import.meta.url)),runtime=readFileSync(new URL('../'+record.runtime,import.meta.url)),a=parse(source),b=parse(runtime),images=new Set(a.g.images.map(i=>i.bufferView));assert.equal(createHash('sha256').update(source).digest('hex'),record.sourceSha256);assert.equal(createHash('sha256').update(runtime).digest('hex'),record.sha256);assert.ok(runtime.length<=1000000&&record.triangles<=10000);assert.equal(record.textures.length,3);assert.ok(record.textures.every(t=>Math.max(...t.size)<=512));assert.ok(b.g.extensionsRequired.includes('EXT_texture_webp'));
  assert.equal(record.normalRepairs.length,['floodlight','workbench'].includes(record.id)?6:0);
  const primitive=a.g.meshes[0].primitives[0],pa=a.g.accessors[primitive.attributes.POSITION],pv=a.g.bufferViews[pa.bufferView],vertex=id=>new Vector3(...[0,1,2].map(k=>a.bin.readFloatLE((pv.byteOffset??0)+(pa.byteOffset??0)+id*(pv.byteStride??12)+k*4)));
  for(const repair of record.normalRepairs){assert.ok(repair.face.includes(repair.vertex));const face=new Triangle(...repair.face.map(vertex)).getNormal(new Vector3());assert.ok(face.dot(new Vector3(...repair.value))>1-1e-6);}
  const na=b.g.accessors[primitive.attributes.NORMAL],nv=b.g.bufferViews[na.bufferView];for(let i=0;i<na.count;i++){const values=[0,1,2].map(k=>b.bin.readFloatLE((nv.byteOffset??0)+(na.byteOffset??0)+i*(nv.byteStride??12)+k*4));assert.ok(values.every(Number.isFinite)&&Math.hypot(...values)>.99);}
  for(let i=0;i<a.g.bufferViews.length;i++)if(!images.has(i)){const x=a.g.bufferViews[i],y=b.g.bufferViews[i],expected=Buffer.from(a.bin.subarray(x.byteOffset??0,(x.byteOffset??0)+x.byteLength));for(const repair of record.normalRepairs.filter(r=>r.bufferView===i)){const accessor=a.g.accessors[repair.accessor],offset=(accessor.byteOffset??0)+repair.vertex*(x.byteStride??12);assert.ok([0,4,8].every(k=>expected.readFloatLE(offset+k)===0));assert.ok(Math.abs(Math.hypot(...repair.value)-1)<1e-6);assert.ok(repair.faceArea2>1e-10);repair.value.forEach((v,k)=>expected.writeFloatLE(v,offset+k*4));}assert.deepEqual(expected,b.bin.subarray(y.byteOffset??0,(y.byteOffset??0)+y.byteLength));}
 }
});
