...
test('the item table covers the worn, held and contextual construction props', () => {
  assert.deepEqual(Object.keys(ITEMS), [
    'rifle-laser', 'sidearm-pistol', 'mining-laser-tool', 'tractor-beam-tool', 'builder-tool',
    'backpack-life-support', 'helmet-standalone',
  ]);
  assert.deepEqual([...HELD_ITEMS], ['rifle-laser', 'sidearm-pistol', 'mining-laser-tool', 'tractor-beam-tool', 'builder-tool']);
...
test('anything that shoots has a unit barrel axis, a muzzle point and a range', () => {
  for (const name of HELD_ITEMS) {
    ...
    if (name === 'tractor-beam-tool' || name === 'builder-tool') { assert.equal(spec.shot, null); continue; }
