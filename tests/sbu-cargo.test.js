...
import { SBU_SIZES,CARGO_GRIDS,crateSize,capacitySBU,placeCrate,validGrid,canRemoveCrate,crateBounds } from '../src/cargo/grid.js';
...
import { FREIGHTER_LAYOUT,LIFTS,FreighterSystems } from '../src/freighter-layout.js';
import { ATLAS_MODEL_URL } from '../src/atlas-gameplay.js';
...
for(const hullName of ['nomad','atlas'])test(`full ${hullName} cargo clears the complete rendered hull and deck details`,async()=>{
 const assetURL=hullName==='atlas'?ATLAS_MODEL_URL:`models/${hullName}.glb`;
 const gltf=await readGLBGeometry(new URL(`../public/${assetURL}`,import.meta.url));let hull=gltf.scene;
 if(hullName==='nomad'){
  ...
  const {createFreighter}=await import('../src/freighter.js'),documentBefore=globalThis.document;
  globalThis.document={createElement:()=>({getContext:()=>new Proxy({},{get:()=>()=>{}})})};
  try{hull=createFreighter(new FreighterSystems(),{gltf});await hull.readyPromise;assert.equal(hull.userData.assetStatus,'ready');}
 ...
 for(let i=0;i<(hullName==='nomad'?6:8);i++){
  const c=placeCrate(hullName,crates,{id:`mesh-${i}`,sbu:hullName==='nomad'?1:64});assert.ok(c);crates.push(c);
 ...
 assert.equal(crates.reduce((n,c)=>n+c.sbu,0),hullName==='nomad'?6:512);assert.equal(validGrid(hullName,crates),true);
 hull.updateMatrixWorld(true);const triangle=new THREE.Triangle();let intersections=0,triangles=0;const hits=new Set();
 hull.traverseVisible(mesh=>{if(!mesh.isMesh)return;const p=mesh.geometry.attributes.position,index=mesh.geometry.index;
  for(let i=0;i<(index?.count??p.count);i+=3){
   ...
   for(const box of boxes)if(box.intersectsTriangle(triangle)){intersections++;hits.add(mesh.name);}
  }
 });assert.ok(triangles>50_000,'the complete production hull was checked, not an empty loader/fallback');
 assert.equal(intersections,0,`no crate bounds intersect authored hull/furniture triangles: ${[...hits].join(', ')}`);
});
test('full cargo keeps Nomad centreline and Atlas crew lift, loading ramps and five-metre centre lane clear',()=>{
 for(const hull of ['nomad','atlas']){
  const crates=[];while(true){const p=placeCrate(hull,crates,{sbu:hull==='nomad'?1:64});if(!p)break;crates.push(p);}
  const boxes=crates.map(c=>crateBounds(hull,c)),layout=hull==='nomad'?SHIP_LAYOUT:FREIGHTER_LAYOUT;
  const systems=hull==='atlas'?new FreighterSystems():null;
  for(let z=hull==='nomad'?-1.2:-23;z<(hull==='nomad'?3.8:23);z+=.1){
   for(const x of hull==='nomad'?[0]:[-2.2,0,2.2]){
    const a=new THREE.Vector3(x,layout.floorY+layout.eyeHeight,z),b=a.clone().add(new THREE.Vector3(0,0,.1));
    assert.deepEqual(constrainShipAttachments(a,b,boxes).toArray(),b.toArray());
    if(hull==='nomad')assert.deepEqual(constrainShipStep(a,b,true).toArray(),b.toArray());
    else assert.deepEqual(systems.constrain(a,b).toArray(),b.toArray());
   }
  }
  if(hull==='atlas'){
   assert.deepEqual(CARGO_GRIDS.atlas.map(g=>[g.id,g.cells]),[['port-deck',[2,8,16]],['starboard-deck',[2,8,16]]]);
   assert.equal(capacitySBU('atlas'),512);assert.deepEqual(LIFTS.map(l=>l.id),['crew']);
   for(const b of boxes){
    assert.ok(b.min[1]>=layout.floorY&&b.max[1]<layout.cargo.ceiling);
    assert.ok(b.max[0]<=-2.5||b.min[0]>=2.5,'cargo leaves the full5m centre lane');
    for(const l of LIFTS)assert.ok(b.max[0]<=l.minX||b.min[0]>=l.maxX||b.max[2]<=l.minZ||b.min[2]>=l.maxZ);
   }
   for(const [x,z] of [[-4.3,-21.5],[5.5,-4],[4.3,21.5]]){
    const a=new THREE.Vector3(0,4.35,z),b=new THREE.Vector3(x,4.35,z);
    assert.deepEqual(constrainShipAttachments(a,b,boxes),b,'fixed cargo does not block a control or crew-lift entrance');
    assert.deepEqual(systems.constrain(a,b),b,'authored furniture and gates leave the same route clear');
   }
  }
 }
});
test('buy validates physical fit and saves no partial oversized delivery',()=>{const s=setup();assert.throws(()=>command(s,'alice',{op:'buy',sbu:8}),/space/);assert.equal(s.accounts.alice.credits,100000);const r=command(s,'alice',{op:'buy',sbu:4});assert.equal(r.state.ships['alice:nomad'].crates.length,1);assert.equal(r.state.accounts.alice.credits,99917);assert.equal(validCommerce(r.state),true);});
test('take allows1SBU only and larger cargo cannot use the retired instant handler',()=>{let s=command(setup(),'alice',{op:'buy',sbu:2}).state;const c=s.ships['alice:nomad'].crates[0];assert.throws(()=>command(s,'bob',{op:'take',ship:'alice:nomad',crate:c.id}),/Only a 1/);assert.throws(()=>command(s,'bob',{op:'haul',ship:'alice:nomad',destination:'bob:atlas',crate:c.id}),/tractor beam/);assert.equal(s.ships['alice:nomad'].crates[0].id,c.id);
 s=command(s,'alice',{op:'buy'}).state;const one=s.ships['alice:nomad'].crates.find(c=>c.sbu===1);s=command(s,'bob',{op:'take',ship:'alice:nomad',crate:one.id}).state;assert.equal(s.accounts.bob.carried.id,one.id);s=command(s,'bob',{op:'stow'}).state;assert.equal(s.accounts.bob.carried,null);assert.equal(s.ships['bob:nomad'].crates[0].id,one.id);
});
...
test('looking at the side of Atlas cargo recognises it while looking down the Nomad aisle leaves hatch control free',()=>{
 ...
 assert.equal(aimedGrid(new THREE.Vector3(1.75,4.35,3),right,pose,'atlas'),true);
...
test('EVA respects closed foreign hulls, real Atlas loading ramps and cargo solids',async()=>{
 ...
 const atlas={hull:'atlas',pose,crates:[],systems:new FreighterSystems()};
 const exterior=new THREE.Vector3(0,4.35,26),interior=new THREE.Vector3(0,4.35,23);
 assert.equal(constrainCargoEVA(exterior,interior,[atlas]).hit,true,'closed aft loading ramp seals the actual64m hull');
 atlas.systems.toggleRamp('aft');for(let i=0;i<200;i++)atlas.systems.update(.02);
 assert.equal(constrainCargoEVA(exterior,interior,[atlas]).hit,false,'fully open aft ramp leaves its real doorway');
 assert.equal(constrainCargoEVA(new THREE.Vector3(0,1.5,4),new THREE.Vector3(0,4.35,4),[atlas]).hit,true,'fixed cargo deck has no retired belly-elevator opening');
 atlas.crates.push(placeCrate('atlas',[],{sbu:64}));
 assert.equal(constrainCargoEVA(new THREE.Vector3(0,4.35,3),new THREE.Vector3(-3,4.35,3),[atlas]).hit,true,'loaded Atlas crate is a real EVA obstacle');
});
