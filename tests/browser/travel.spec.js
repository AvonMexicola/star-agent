...
test('controller map bearing, charge, abort, direct-sight moon arrival and return to flight',async({page,browser})=>{
 ...
 await page.waitForFunction(()=>window.starAgent.state.moon.lod>=7&&window.starAgent.state.moon.pending===0,undefined,{timeout:15000});for(let i=0;i<15;i++)await frames(page);await page.screenshot({path:`${evidence}/moon-arrival.png`});await button(0,true);await page.waitForFunction(()=>window.starAgent.state.speed>1);await button(0,false);await tap(6);
