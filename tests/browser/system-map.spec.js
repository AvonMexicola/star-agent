...
test('hierarchy centres stars and planets; sites and filters fit desktop and phone without scrolling',async({page,browser})=>{
 ...
 for(const size of [{width:1440,height:900},{width:390,height:844}]){
  ...
  for(const id of ['star','aeon','selene','pyre','miasma']){
   if(size.width<750)await page.locator('[data-map-view="chart"]').click();
   ...
   for(const view of ['locations','signals','filters']){await page.locator(`[data-map-view="${view}"]`).click();await frames(page);findings.push({size,id,view,...await fits(page)});}
  }
 }
...
test('controller reaches real surface and mission signals, toggles filters and returns safely',async({page,browser})=>{
 ...
 await page.waitForFunction(()=>window.starAgent.state.controller.armed);await expect(page.locator('.navigation-marker[data-id="mission-patrol"]')).toBeVisible();
