...
const evidence=process.env.TOOL_ART_EVIDENCE||'/tmp/star-agent-tool-visual-evidence';
...
test('HDR log-depth shows textured tool and beam ending at target with correct occlusion',async({page,browser})=>{
  ...
  const meta=await page.evaluate(()=>window.visual.meta());expect(meta.logDepth).toBe(true);expect(meta.textures).toBe(3);
