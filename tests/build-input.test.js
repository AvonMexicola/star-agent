...
test('triggers rotate on edges, LB snaps, RB jumps and D-pad adjusts height',()=>{
  ...
  f.press(4,true);f.press(12,true);f.press(5,true);frame=f.poll();assert.deepEqual(f.calls.slice(-2),[['cycleSnap'],['adjustHeight',.25]]);assert.equal(frame.jump,true);assert.equal(frame.roll,0);assert.equal(f.poll().jump,false);
