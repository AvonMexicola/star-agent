...
class SynchronousWorker {
  postMessage(job){const cut=job.point?carve(job.field,job.point,job.budget,.48,job.resourceWeights):{field:job.field,yieldVolume:[0,0,0]};
...
test('third-person inspection reports and enforces reach from the physical player eye',()=>{
  ...
  const field={cache:new Map(),rings:{raycast:()=>null},fieldCache:{raycast:()=>null},store:{canEditRock:()=>true},
...
test('ring rock promotion mines into the shared backpack and restores its cut after streaming away',t=>{
  ...
  const hit=field.raycast(origin,direction);assert.ok(hit?.rock.space);const id=hit.rock.rockId;assert.match(id,/^selene-ring-v2-/);
