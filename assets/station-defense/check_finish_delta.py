"""Strict standard-library GLB geometry/rig delta for the Bastion07 enamel pass.

python assets/station-defense/check_delta.candidate-07.py --before /archive/candidate-06.glb \
  --after public/models/station-defense.glb --out /qa/delta-06-07.json

Only TEXCOORD_0 and embedded texture payloads may change. Expanded oriented
local/world triangles, normals and every other corner attribute must agree.
"""
...
checks={
    'baselineIdentity':before['sha256']=='adf6c5b03da18710a3d97341dd503ec95433e564d81610ed67037b226af8e4ca',
    ...
    'allCornerNormalsIdentical':before['normalCorners']==after['normalCorners'],
...
report={'result':'PASS' if all(checks.values()) else 'FAIL','checks':checks,
        ...
        'scope':'Exact expanded oriented triangle coordinates in local and world space, full rig records and all non-UV corner data including normals. Only UV0 and embedded texture payload changes are allowed. No browser or independent appearance acceptance.'}
