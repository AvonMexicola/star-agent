# pyrebear source pack

User-supplied Meshy quadruped for Pyre; source generation prompt/settings were
not supplied. This task performed no hosted generation or credit spending.

- `source/pyrebear-walking.glb`: exact supplied walking/skinned GLB.
- `source/albedo.png`: exact embedded source image, extracted for encoding.
- `pyrebear-albedo.webp`: derived 1024-edge, quality88 runtime albedo.
- `pyrebear.blend`: editable validated runtime rig with walk/death actions.
...
Rebuild from the repository using `blender/finish_pyrebear.py -- --species pyrebear`
under headless Blender, with `ALSOFT_DRIVERS=null` and `--python-exit-code 1`.
The approved phase-zero grounded withers height is 1.65m.
