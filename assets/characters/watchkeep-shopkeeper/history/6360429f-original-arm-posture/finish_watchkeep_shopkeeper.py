...
"""
import argparse,copy,hashlib,json,math,struct,subprocess,sys,zipfile
...
def preservation_receipt(doc,binary,clips):
    ...
    for label in LABELS:
        ...
        for old,new in zip(before['channels'],exported['channels']):
            ...
            hashes={}
            for key in ['input','output']:
                ab=view_bytes(original,raw,original['accessors'][a[key]]['bufferView']);bb=view_bytes(doc,binary,doc['accessors'][b[key]]['bufferView']);assert ab==bb
                hashes[key+'Sha256']=hashlib.sha256(ab).hexdigest()
            tracks.append({'node':original['nodes'][old['target']['node']]['name'],'path':old['target']['path'],**hashes})
        ...
        receipt.append({'name':exported['name'],'originalChannelsPreserved':len(tracks),'addedChannels':['Watchkeep_normalized_root.translation'],'inverseBindSha256':hashlib.sha256(a).hexdigest(),'tracks':tracks})
    (PACK/'source-preservation.json').write_text(json.dumps(receipt,indent=2)+'\n');return {'originalAnimationChannelsPerClip':72,'originalAnimationFloatBuffers':'byte-identical','originalNodesAndSkin':'identical before one outer normalization node','inverseBindBuffer':'byte-identical','geometry':'derived protected reduction; source archive unchanged'}
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--skip-renders',action='store_true');parser.add_argument('--render-only',action='store_true');parser.add_argument('--verify-reproducible',action='store_true');args=parser.parse_args(sys.argv[sys.argv.index('--')+1:] if '--'in sys.argv else [])
    if args.render_only:
    ...
    binary=replace_geometry(doc,binary,attributes,indices);derived=PACK/'derived-unfitted.glb';write_glb(derived,doc,binary);arm,mesh=load_scene(derived);activate('idle-15');frame(0)
